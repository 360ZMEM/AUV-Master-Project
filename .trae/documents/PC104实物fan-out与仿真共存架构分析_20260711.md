...
推荐把 fan-out 拆成“环境文件 + service 文件”两部分，避免把 IP、端口、订阅者写死在 systemd unit 中。

环境文件建议路径：

```bash
/etc/auv/pc104-fanout.env
```
...
# /etc/auv/pc104-fanout.env
# PC104 实物端
PC104_HOST=192.168.0.101
PC104_PORT=21

# 上位机实物网卡端
LISTEN_HOST=192.168.0.11
LISTEN_PORT=21
...
# 本地消费者
ROS_SUBSCRIBER=ros2=127.0.0.1:52365
PYSIDE6_SUBSCRIBER=pyside6=127.0.0.1:52366
ROS_SOURCE_PORT=52365
CONSOLE_SOURCE_PORT=52366

# 默认禁止非零执行器。需要短测时应临时新建 bench service 或手动显式覆盖。
...
```bash
/etc/systemd/system/pc104-fanout.service
```
...
```ini
[Unit]
Description=AUV PC104 UDP fan-out proxy
Documentation=file:/home/gwxie/master_work-tmp/AUV_Master_Project/.trae/documents/PC104实物fan-out与仿真共存架构分析_20260711.md
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
EnvironmentFile=/etc/auv/pc104-fanout.env
WorkingDirectory=/home/gwxie/master_work-tmp/AUV_Master_Project
ExecStart=/usr/bin/python3 /home/gwxie/master_work-tmp/AUV_Master_Project/scripts/pc104_udp_fanout.py \
  --listen-host ${LISTEN_HOST} \
  --listen-port ${LISTEN_PORT} \
  --pc104-host ${PC104_HOST} \
  --pc104-port ${PC104_PORT} \
  --cmd-host ${CMD_HOST} \
  --cmd-port ${CMD_PORT} \
  --subscriber ${ROS_SUBSCRIBER} \
  --subscriber ${PYSIDE6_SUBSCRIBER} \
  --ros-source-port ${ROS_SOURCE_PORT} \
  --console-source-port ${CONSOLE_SOURCE_PORT}
Restart=on-failure
RestartSec=2
KillSignal=SIGINT
TimeoutStopSec=5

# fan-out 需要绑定 21/udp。开发期可用 root 运行；工业部署更建议配合
# AmbientCapabilities 仅授予低端口绑定能力，而不是让业务逻辑依赖交互式 sudo。
User=root
Group=root

# 基础加固。若现场需要写日志到文件，应放开对应目录。
NoNewPrivileges=true
PrivateTmp=true
ProtectSystem=full
ProtectHome=read-only
ReadWritePaths=/home/gwxie/master_work-tmp/AUV_Master_Project/.runtime
...
如果目标系统支持 Linux capability，更推荐将 service 改为普通用户运行，并只授予低端口绑定能力：
...
这种方式比 `sudo python3 ...` 更适合工业现场，因为：

- fan-out 进程不需要完整 root 权限。
- 现场用户不需要知道 sudo 密码。
- systemd 统一负责进程生命周期、重启、日志和退出清理。

部署 SOP：

```bash
sudo mkdir -p /etc/auv
sudo cp /path/to/pc104-fanout.env /etc/auv/pc104-fanout.env
sudo cp /path/to/pc104-fanout.service /etc/systemd/system/pc104-fanout.service
sudo systemctl daemon-reload
sudo systemctl enable pc104-fanout.service
sudo systemctl start pc104-fanout.service
sudo systemctl status pc104-fanout.service
```

运行期观察：

```bash
journalctl -u pc104-fanout.service -f
ss -lunp 'sport = :21'
ss -lunp 'sport = :52364'
```
...
- `192.168.0.11:21/udp` 被 fan-out 持有。
- `127.0.0.1:52364/udp` 被 fan-out 持有。
- 日志中出现 `PC104 uplink bind ...`、`local downlink bind ...`。
- 实物通电且 PC104 正常发包时，日志中 `uplink=...` 持续增长。

停止 SOP：

```bash
sudo systemctl stop pc104-fanout.service
systemctl status pc104-fanout.service
ss -lunp 'sport = :21'
ss -lunp 'sport = :52364'
```
...
- `pc104-fanout.service` 进入 inactive/dead。
- `21/udp` 和 `52364/udp` 释放。
- ROS2/PySide6 不再收到新的 PC104 上行复制帧。

非零执行器短测不建议直接修改常驻 service。推荐单独建立 `pc104-fanout-bench.service` 或临时手动启动：

```bash
sudo /usr/bin/python3 scripts/pc104_udp_fanout.py ... --allow-nonzero-actuator
```

这样可以避免常驻工业服务意外长期放行非零执行器。bench service 的命名、日志和启停权限也应与常规 `pc104-fanout.service` 区分。
...
- 权限边界清晰。
- 日志可以进 journald。
- 可通过 `Restart=on-failure` 做进程级自恢复。
- 可通过 `AmbientCapabilities=CAP_NET_BIND_SERVICE` 避免整进程 root 化。
- 可把现场 SOP 固化为 `systemctl start/status/stop`，减少误操作。

缺点：

- 需要部署 service 文件。
- 需要定义配置路径、日志策略和重启策略。
- 需要系统管理员预先安装 unit 和环境文件。
- 如果使用 root 运行，仍需额外审查脚本读写边界。
- 如果使用 capability，需确认目标 Linux 发行版和 systemd 版本支持该配置。
