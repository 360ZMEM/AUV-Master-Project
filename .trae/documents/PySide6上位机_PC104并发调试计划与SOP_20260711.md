...
截至 2026-07-11 16:20，自动化验证状态：

- fan-out + ROS2 passive 实物短测已通过。
- fan-out + ROS2 non-passive 零执行器 180 秒 soak 已通过。
- 普通用户 ROS CLI 已可读取普通用户 ROS2 bridge 的 topic，规避了此前 sudo-launched bridge 的 DDS 上下文问题。
- fan-out 日志确认 ROS2 下行保持 `obj=1`、安全保护参数 `500/29/300/200/10`、电机和舵角全 `0`。
- 停止脚本确认 `21/udp`、`52364/udp`、`52365/udp`、`52366/udp` 均可释放。
- Zenoh side channel 仍未闭合：`eclipse-zenoh` 全局安装卡在 metadata 阶段，`sudo /usr/bin/python3 import zenoh` 仍失败。

当前可直接使用的脚本：

```bash
# 启动并发栈，默认 ROS2 passive
scripts/start_pc104_fanout_concurrent.sh --passive

# 启动并发栈，ROS2 non-passive 零执行器
...
# 带 timeout 的 ROS2 non-passive 零执行器 soak
...
1. 确认已安装 Python `zenoh`。
   - 当前状态：普通用户目录已有 `eclipse-zenoh==1.8.0`，但全局 sudo Python 仍不能 `import zenoh`。
   - 因此该 SOP 暂时保留，不作为主链路门禁。
