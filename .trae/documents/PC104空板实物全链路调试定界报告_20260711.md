...
结论状态：Telnet、UDP、上位机直连、ROS2 被动上行链路、ROS2 非 passive 零执行器下行链路均已形成可复现实物证据。仍未闭合的是 BUG-4 Bit9、DVL 硬限/丢底自救主推输出、真实传感器链和带负载执行机构闭环。
...
ROS2 `protocol_udp` 非 passive idle 烟测已经在补完安全默认保护参数并重新构建 `auv_bridge` 后完成。短测期间执行器输出保持为零，PC104 影子保护参数保持 `500/29/300/200/10`。需要特别记录的是：第一次短测前忘记重建 `brain_linux/install`，导致旧代码仍把 `Depth_Para1` 写成 0；已立即用上位机安全帧恢复，并在重建后复测通过。
...
### 2.8 ROS2 `protocol_udp` 非 passive 零执行器下行

已完成源码修复：

- `CommandArbiter` 新增可配置默认 remote 保护参数：
  - `default_depth_protect_params`
  - `default_bottom_protect_params`
  - `default_preset_time_tenths_min`
- `AUVBridgeNode` 从 `bridge.arbiter.default_remote_payload` 读取上述参数。
- `brain_linux/config/params.protocol_udp_pc104.yaml` 显式配置：
...
- 语法检查通过。
- `test_arbiter.py` 定向单测通过，结果为 `6 passed`。
- dry-run 构包确认：
  - payload depth 为 `(500, 29)`
  - payload bottom 为 `(300, 200)`
  - payload preset 为 `10`
  - encoded packet 为 `obj=1`、`ctrl=1`、`work=0`、`motor=0/0`

实物短测过程：

- 第一次尝试直接运行 `brain_linux/install` 中旧代码，PC104 读回 `UI depth_para1=0`。
- 根因：源码已改，但 ROS2 install 尚未重新构建。
- 已立即发送上位机安全恢复帧，把 `UI depth_para1` 恢复到 `500`。
- 执行 `colcon build --packages-select auv_bridge --symlink-install` 后复测。
- 第二次非 passive 短测使用：
  - `passive_mode:=false`
...
- bridge 日志显示：
  ...
  - 短测窗口内收到 `30/60/90` 个真实上行 payload。
- 短测后 `21/udp` 已释放。
...
结论：ROS2 `protocol_udp` 非 passive idle 下行已经闭合在“零执行器、安全保护参数”的短时实物烟测范围内。
...
### 3.4 ROS2 非 passive 的遗留边界

- ROS2 `protocol_udp` 非 passive idle 短测已经通过，但范围仅限“零执行器、安全保护参数、短时间窗口”。
- 尚未验证 MPC 或 `/cmd_vel` 产生非零推力/舵角后的真实执行机构链路。
- 尚未验证长时间运行下的丢包恢复、DDS 用户上下文、bridge 重启恢复和 PC104 watchdog 边界。
- 教训：修改 ROS2 源码后必须重新 `colcon build` 并重新 source `brain_linux/install/setup.bash`，否则实物运行的仍可能是旧 install 代码。
...
### 5.6 Zenoh Python 依赖缺失

- ROS2 `protocol_udp` 实物短测日志出现：
  - `zenoh python package unavailable, disable protocol_udp side channel: No module named 'zenoh'`
- 本次 PC104 `protocol_udp` 收发主链路不依赖该 side channel，因此不影响 `$AUV` 上行、`$CKTH` 下行和 ROS2 topic 验证。
- 但如果后续要同时验证 Zenoh side channel、PC raw command side channel 或仿真/实物混合链路，需要安装并固定 `zenoh` Python 依赖。
- TODO：
  - 在 ROS2/PC104 启动前依赖检查中加入 `python3 -c "import zenoh"`。
  - 明确 side channel 不可用时哪些功能会降级。
...
ROS2 PC104 profile/arbiter 默认 remote payload 安全参数已经补完，非 passive 零执行器短测已经通过。后续若继续推进更高风险测试，应按以下门禁执行：

1. 非零 `/cmd_vel` 或 MPC 输出测试前：
   - 必须明确是否允许推进器/舵机动作。
   - 必须先在 PC104 侧确认执行机构电源、负载和急停状态。
   - 必须使用逐级递增的小幅命令，不允许直接上大推力。

2. 长时间 ROS2 非 passive 运行前：
   - 启动前确认 `21/udp` 未占用。
   - 启动后记录 payload 计数、Telnet shadow、`/auv/arbiter/status`。
   - 停止后确认 `21/udp` 释放。
   - 若源码有改动，必须先 `colcon build --packages-select auv_bridge --symlink-install`。

3. 接入真实传感器前：
   - 先单传感器验链，再进入融合链路。
   - DVL、深度计、IMU、BMS/漏水应分别留出独立验收记录。
   - 不应把 Telnet 注入结果等同于真实传感器验收。

4. 固件修复后：
   - 重点复验 BUG-4 Bit9。
   - 重点复验 BUG-5/BUG-6 的自救主推输出是否能保留到 `Instruction_To_FMCU` 和 `$MCUFD`。
