...
void Unpack_Data_From_DVL_ACK(u8 *temp_buf);/*岸上只输出ack自检语句*/
...
float Calculate_Location(void);/*这里的void要改成带参数运算的函数*/
...
void Current_state(_Current_State *temp)
{
	...
	if(Parameter_Adjustment_Flag == true)
	{
		...
		/*先推算航位*/		
		if(BI_Cal_Data_Flag == true)/*对底，推算下一个点的经纬度*/
		{		   
		    if(Recv_From_GPS_QC_Flag == true)/*收到gps数据有效就开始推算，收不到gps数据就不推算了*/
			{
		...
		if(WI_Cal_Data_Flag == true)/*对水，推算下一个点的经纬度*/		
		{
		    if(Recv_From_GPS_QC_Flag == true)/*收到gps数据就开始推算，收不到gps数据就不推算了*/
			{
		    	   One_Point_Longitude = GPS_Prase_Data.GPS_Longtitude;
		    	   One_Point_Latitude = GPS_Prase_Data.GPS_Latitude;
...
void Unpack_Data_From_FMCU(u8 *temp_buf)
{
	...
	/*接收之后，立马就是McuFD_Power_State，McuFD_Sys_Abnorm_Inf，McuFD_Dev_Abnorm_Inf，McuFD_Dev_Abnorm_Inf_Detail判断状态*/
	
	/*解析Data_From_FMCU.McuFD_Power_State*/
	if((Data_From_FMCU.McuFD_Power_State & 0x0001) == 0x0001)    /*bit0:  主推上电*/
	{
	...
	if((Data_From_FMCU.McuFD_Power_State & 0x0002) == 0x0002)  /*bit1: 侧推上电*/
	{
	...
	if((Data_From_FMCU.McuFD_Power_State & 0x0004) == 0x0004)/*bit2: 水平舵上电*/
	{
	...
	if((Data_From_FMCU.McuFD_Power_State & 0x0008) == 0x0008)/*bit3: 垂直舵上电*/
	{
	...
	if((Data_From_FMCU.McuFD_Power_State & 0x0010) == 0x0010)  /*Bit4: 主电源应急压载上电*/
	{
	...
	if((Data_From_FMCU.McuFD_Power_State & 0x0020) == 0x0020) /*Bit5: 备用能源应急压载上电*/
	{
	...
	if((Data_From_FMCU.McuFD_Power_State & 0x0040) == 0x0040)  /*Bit6: 主电源通信模块上电*/
	{
	...
	if((Data_From_FMCU.McuFD_Power_State & 0x0080) == 0x0080)  /*Bit7: 备用能源通信模块上电*/
	{	
	...
	if((Data_From_FMCU.McuFD_Power_State & 0x0100) == 0x0100)    /*Bit8: 多普勒计程仪（DVL）上电*/
	{
	...
	if((Data_From_FMCU.McuFD_Power_State & 0x0200) == 0x0200)  /*Bit9：备用1上电*/
	{
	...
	if((Data_From_FMCU.McuFD_Power_State & 0x0400) == 0x0400)/*Bit10: 备用2上电*/
	{
...
void Remote_Assignment(_To_MCUFD *temp)
{
	...
	((*temp).McuFD_Msg_Num)++;
	if((temp->McuFD_Msg_Num)>254)/*到255重置为0*/
	{
	...
	if(Course_Keep_Flag == true)
	{
		...
		Course_Keep_UV_Set_Rud_Angle=Course_Keep_Algorithm(Course_set_angle, Current_State.Current_IMU_Heading, IMU_Prase_Data.AngRateX_AngRateY_AngRateZ[2]);/*预留待写,要修改*/
		Course_Keep_LV_Set_Rud_Angle=Course_Keep_Algorithm(Course_set_angle, Current_State.Current_IMU_Heading, IMU_Prase_Data.AngRateX_AngRateY_AngRateZ[2]);/*预留待写,要修改*/		
...
void Pack_Data_To_UI12(_To_UI12 *temp)
{
	...
	/*****************test para***********/
#if 0
	 temp->ToUI12_Msg_Length=145;/*本来是144，加上结束符变成145*/
	...
	To_UI12_Buf[8] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[9] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = (u16)(temp->ToUI12_Depth_Para2);        
	To_UI12_Buf[10] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[11] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = (u16)(temp->ToUI12_Height_Para1);        
	To_UI12_Buf[12] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[13] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = (u16)(temp->ToUI12_Height_Para2);        
	To_UI12_Buf[14] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[15] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = (u16)(temp->ToUI12_Remain_Time);        
	To_UI12_Buf[16] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[17] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	To_UI12_Buf[22] = temp->ToUI12_Work_Cmd;     
	
	convert_u16_data1 = temp->ToUI12_Motor_Speed1;        
	To_UI12_Buf[23] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[24] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = temp->ToUI12_Motor_Speed2;        
	To_UI12_Buf[25] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[26] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = temp->ToUI12_HL_Rud_Angle;        
	To_UI12_Buf[27] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[28] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = temp->ToUI12_HR_Rud_Angle;        
	To_UI12_Buf[29] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[30] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = temp->ToUI12_VU_Rud_Angle;        
	To_UI12_Buf[31] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[32] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = temp->ToUI12_VL_Rud_Angle;        
	To_UI12_Buf[33] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[34] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = temp->ToUI12_Pres;        
	To_UI12_Buf[35] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[36] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	To_UI12_Buf[37] = temp->ToUI12_Temp;    
	
	convert_u16_data1 = (u16)(temp->ToUI12_Depth);        
	To_UI12_Buf[38] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[39] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	
	convert_u32_data2 = htonl((u32)(temp->ToUI12_Para1));        /*转化为网络字节序*/
		for(cycle_number=0; cycle_number<4; cycle_number++)
			To_UI12_Buf[40+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
		
		
	convert_u32_data2 = htonl((u32)(temp->ToUI12_Para2));        /*转化为网络字节序*/
		for(cycle_number=0; cycle_number<4; cycle_number++)
			To_UI12_Buf[44+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
			
		
	convert_u32_data2 = htonl((u32)(temp->ToUI12_Para3));        /*转化为网络字节序*/
		for(cycle_number=0; cycle_number<4; cycle_number++)
			To_UI12_Buf[48+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
			
		
	convert_u32_data2 = htonl((u32)(temp->ToUI12_Para4));        /*转化为网络字节序*/
		for(cycle_number=0; cycle_number<4; cycle_number++)
			To_UI12_Buf[52+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
			
		
	convert_u16_data1 = (u16)(temp->ToUI12_Para5);        
	To_UI12_Buf[56] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[57] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
	
	convert_u16_data1 = (u16)(temp->ToUI12_Para6);        
	To_UI12_Buf[58] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[59] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
					
	
	convert_u16_data1 = (u16)(temp->ToUI12_Para7);        
	To_UI12_Buf[60] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[61] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	
	convert_u16_data1 = (u16)(temp->ToUI12_Para8);        
	To_UI12_Buf[62] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[63] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
	
	convert_u16_data1 = (u16)(temp->ToUI12_Para9);        
	To_UI12_Buf[64] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[65] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
	
	convert_u16_data1 = (u16)(temp->ToUI12_Para10);        
	To_UI12_Buf[66] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[67] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
	
	convert_u16_data1 = (u16)(temp->ToUI12_Para11);        
	To_UI12_Buf[68] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[69] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
					
	
	convert_u16_data1 = (u16)(temp->ToUI12_Para12);        
	To_UI12_Buf[70] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[71] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
				

	convert_u16_data1 = (u16)(temp->ToUI12_IMU_Heading);        
	To_UI12_Buf[72] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[73] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = temp->ToUI12_IMU_Pitch;        
	To_UI12_Buf[74] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[75] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = temp->ToUI12_IMU_Roll;        
	To_UI12_Buf[76] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[77] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = (u16)(temp->ToUI12_GPS_Heading);        
	To_UI12_Buf[78] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[79] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = (u16)(temp->ToUI12_GPS_Velocity);        
	To_UI12_Buf[80] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[81] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = temp->ToUI12_DVL_Velocity;        
	To_UI12_Buf[82] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[83] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = (u16)(temp->ToUI12_Height);        
	To_UI12_Buf[84] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[85] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u32_data2 = htonl((temp->ToUI12_Cal_Longitude));        /*转化为网络字节序*/
	for(cycle_number=0; cycle_number<4; cycle_number++)
		To_UI12_Buf[86+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
	
	convert_u32_data2 = htonl((temp->ToUI12_Cal_Latitude));        /*转化为网络字节序*/
	for(cycle_number=0; cycle_number<4; cycle_number++)
		To_UI12_Buf[90+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
	
	convert_u32_data2 = htonl((temp->ToUI12_GPS_Longitude));        /*转化为网络字节序*/
	for(cycle_number=0; cycle_number<4; cycle_number++)
		To_UI12_Buf[94+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
	
	convert_u32_data2 = htonl((temp->ToUI12_GPS_Latitude));        /*转化为网络字节序*/
	for(cycle_number=0; cycle_number<4; cycle_number++)
		To_UI12_Buf[98+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
	
	convert_u16_data1 = (u16)(temp->ToUI12_Total_Voltage);        
	To_UI12_Buf[102] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[103] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = (u16)(temp->ToUI12_Total_Current);        
	To_UI12_Buf[104] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[105] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	To_UI12_Buf[106] = temp->ToUI12_SOC;   
	To_UI12_Buf[107] = temp->ToUI12_SOH;   
	
	convert_u16_data1 = (u16)(temp->ToUI12_SingleMax_Voltage);        
	To_UI12_Buf[108] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[109] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	convert_u16_data1 = (u16)(temp->ToUI12_SingleMin_Voltage);        
	To_UI12_Buf[110] = convert_u16_data1>>8;/*相当于取高八位*/
	To_UI12_Buf[111] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
	
	To_UI12_Buf[112] = temp->ToUI12_SingleMax_Temp;   
	To_UI12_Buf[113] = temp->ToUI12_SingleMin_Temp;   
	
	convert_u32_data2 = htonl((u32)(temp->ToUI12_DevicePower_State));        /*转化为网络字节序*/
	for(cycle_number=0; cycle_number<4; cycle_number++)
		To_UI12_Buf[114+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
	
	convert_u32_data2 = htonl((u32)(temp->ToUI12_Cmd_State));        /*转化为网络字节序*/
	for(cycle_number=0; cycle_number<4; cycle_number++)
		To_UI12_Buf[118+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
	
	convert_u32_data2 = htonl((u32)(temp->ToUI12_Sail_State));        /*转化为网络字节序*/
	for(cycle_number=0; cycle_number<4; cycle_number++)
		To_UI12_Buf[122+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
	
	convert_u32_data2 = htonl((u32)(temp->ToUI12_Sys_Abnorm_Inf));        /*转化为网络字节序*/
	for(cycle_number=0; cycle_number<4; cycle_number++)
		To_UI12_Buf[126+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
	
	convert_u32_data2 = htonl((u32)(temp->ToUI12_Dev_Abnorm_Inf));        /*转化为网络字节序*/
	for(cycle_number=0; cycle_number<4; cycle_number++)
		To_UI12_Buf[130+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
	
	convert_u32_data2 = htonl((u32)(temp->ToUI12_BMS_Abnorm_Inf));        /*转化为网络字节序*/
	for(cycle_number=0; cycle_number<4; cycle_number++)
		To_UI12_Buf[134+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
	
	convert_u32_data2 = htonl((u32)(temp->ToUI12_Dev_Abnorm_Inf_Detail));        /*转化为网络字节序*/
	for(cycle_number=0; cycle_number<4; cycle_number++)
...
void Unpack_Data_From_DVL_BI(u8 *temp_buf)
{
	   ...
	   /* char i[1];	用来接收有效标志位A
		...
		memcpy(DVL_Prase_Data.head_buf, temp_buf, 3);          /*帧头*/
		ptr = strstr(ptr+1, ",");
		DVL_Prase_Data.BI_X = atoi(ptr+1);   /*X轴速度*/
		ptr = strstr(ptr+1, ","); 
		DVL_Prase_Data.BI_Y = atoi(ptr+1);   /*Y轴速度*/
		ptr = strstr(ptr+1, ","); 
		DVL_Prase_Data.BI_Z = atoi(ptr+1);   /*Z轴速度*/
		...
		/*ptr = strstr(ptr+1, ","); 		
		ptr = strstr(ptr+1, ","); 
		strncpy(i, ptr+1, strlen(ptr+1));  有效标志位
		...
		/*DVL_Prase_Data.BI_Valid_Flag=true;*/
		memset(temp_buf, 0, From_DVL_BI_Length);/*清空temp_buf*/	
...
void Unpack_Data_From_DVL_WI(u8 *temp_buf)
{
	...
	memcpy(DVL_Prase_Data.head_buf, temp_buf, 3);          /*帧头*/
	ptr = strstr(ptr+1, ",");
	DVL_Prase_Data.WI_X = atoi(ptr+1);   /*X轴速度*/
	ptr = strstr(ptr+1, ","); 
	DVL_Prase_Data.WI_Y = atoi(ptr+1);   /*Y轴速度*/
	ptr = strstr(ptr+1, ","); 
	DVL_Prase_Data.WI_Z = atoi(ptr+1);   /*Z轴速度*/
...
{
	char *ptr = (char *)temp_buf;
	memcpy(GetACKfromDVL.head_buf, temp_buf, 4);          /*帧头*/
	ptr = strstr(ptr+1, ",");
	GetACKfromDVL.F_selftest= atof(ptr+1); /*F代表自检模式响应*/		
	ptr = strstr(ptr+1, ",");
	GetACKfromDVL.A_channel= atof(ptr+1);  /*A,B,C,D代表四个通道*/	
	ptr = strstr(ptr+1, ",");
	GetACKfromDVL.B_channel= atof(ptr+1);  /*A,B,C,D代表四个通道*/	
	ptr = strstr(ptr+1, ",");
	GetACKfromDVL.C_channel= atof(ptr+1);  /*A,B,C,D代表四个通道*/		
	ptr = strstr(ptr+1, ",");
	GetACKfromDVL.D_channel= atof(ptr+1);  /*A,B,C,D代表四个通道*/		
	memset(temp_buf, 0, strlen((char *)temp_buf));	
...
void Unpack_Data_From_GPS_GGA(u8 *temp_buf)
{
	...
	ptr = strstr((char *)temp_buf, ",");
	GPS_Prase_Data.UTC_Time = atof(ptr+1);                 /*1 hhmmss.ss,取出时分秒点秒*/
	...
	ptr = strstr(ptr+1, ",");                  /*6*/
	GPS_Prase_Data.GPS_Position_QC = atoi(ptr + 1);  /*GPS状态：0故障，1有效*/
	...
	if((GPS_Prase_Data.GPS_Position_QC & 0x01) == 0x01)   /*如果GPS状态有效*/      
	{
		Recv_From_GPS_QC_Flag = true;/*如果GPS状态有效,则可以用于航位推算*/
		
		if(GPS_Prase_Data.UTC_Time > 0)/*如果UTC时间有效*/  
		{
	...
	if((GPS_Prase_Data.GPS_Position_QC & 0x01) != 0x01)   /*如果GPS状态无效*/    
	{
		Not_Recv_From_GPS_No++;/*如果GPS状态有效,则计数，Security.c就会将Recv_From_GPS_QC_Flag置true*/
	}
...
void Unpack_Data_From_GPS_VTG(u8 *temp_buf)
{
	...
	ptr = strstr(ptr+1, ","); 
	ptr = strstr(ptr+1, ","); 
	GPS_Prase_Data.GPS_Course = atof(ptr+1);   /*取出对地磁北航向，单位为度*/
	ptr = strstr(ptr+1, ","); 
	ptr = strstr(ptr+1, ","); 
	GPS_Prase_Data.GPS_Velocity_Kn = atof(ptr+1);   /*取出对地速度，单位是节*/
	ptr = strstr(ptr+1, ","); 
	ptr = strstr(ptr+1, ","); 	
	GPS_Prase_Data.GPS_Velocity_Kmph = atof(ptr+1);  /*取出对地速度，单位是千米每小时*/	
	memset(temp_buf, 0, strlen((char *)temp_buf));
}
...
void Pack_Data_To_UI3(_ToUI3 *temp)
{
	...
	temp->ToUI3_Back_Longitude=Current_State.Back_Lon;/*协议上面写的Back_Lon，需要确定一下是不是Current_Cal_Longitude，还是需要在结构体里面重新定义一个Back_Lon*/
	temp->ToUI3_Back_Latitude=Current_State.Back_Lat;/*协议上面写的Back_Lat，需要确定一下是不是Current_Cal_Latitude，还是需要在结构体里面重新定义一个Back_Lat*/
	...
	/*****************test para***********/
	 ...
	 /***********************************/
		...
		To_UI3_Buf[8] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[9] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
		convert_u16_data1 = (u16)(temp->ToUI3_Depth_Para2);        
		To_UI3_Buf[10] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[11] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
		convert_u16_data1 = (u16)(temp->ToUI3_Height_Para1);        
		To_UI3_Buf[12] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[13] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
		convert_u16_data1 = (u16)(temp->ToUI3_Height_Para2);        
		To_UI3_Buf[14] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[15] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
		convert_u16_data1 = (u16)(temp->ToUI3_Remain_Time);        
		To_UI3_Buf[16] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[17] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
		convert_u16_data1 = (u16)(temp->ToUI3_Spare_Para1);        
		To_UI3_Buf[18] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[19] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/		
		
		convert_u16_data1 = (u16)(temp->ToUI3_Spare_Para2);        
		To_UI3_Buf[20] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[21] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/	
		
		convert_u32_data2 = htonl((temp->ToUI3_Back_Longitude));        /*转化为网络字节序*/
		for(cycle_number=0; cycle_number<4; cycle_number++)
			To_UI3_Buf[22+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
		
		convert_u32_data2 = htonl((temp->ToUI3_Back_Latitude));        /*转化为网络字节序*/
		for(cycle_number=0; cycle_number<4; cycle_number++)
			To_UI3_Buf[26+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
		
		convert_u16_data1 = temp->ToUI3_Pres;        
		To_UI3_Buf[30] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[31] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
		To_UI3_Buf[32] = temp->ToUI3_Temp;    
		
		convert_u16_data1 = (u16)(temp->ToUI3_Depth);        
		To_UI3_Buf[33] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[34] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
		convert_u16_data1 = (u16)(temp->ToUI3_IMU_Heading);        
		To_UI3_Buf[35] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[36] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
		convert_u16_data1 = temp->ToUI3_IMU_Pitch;        
		To_UI3_Buf[37] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[38] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
		convert_u16_data1 = temp->ToUI3_IMU_Roll;        
		To_UI3_Buf[39] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[40] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
		convert_u16_data1 = (u16)(temp->ToUI3_GPS_Heading);        
		To_UI3_Buf[41] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[42] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
		convert_u16_data1 = (u16)(temp->ToUI3_GPS_Velocity);        
		To_UI3_Buf[43] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[44] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
		convert_u16_data1 = temp->ToUI3_DVL_Velocity;        
		To_UI3_Buf[45] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[46] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
		convert_u16_data1 = (u16)(temp->ToUI3_Height);        
		To_UI3_Buf[47] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[48] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
		convert_u32_data2 = htonl((temp->ToUI3_GPS_Longitude));        /*转化为网络字节序*/
		for(cycle_number=0; cycle_number<4; cycle_number++)
			To_UI3_Buf[49+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 

		convert_u32_data2 = htonl((temp->ToUI3_GPS_Latitude));        /*转化为网络字节序*/
		for(cycle_number=0; cycle_number<4; cycle_number++)
			To_UI3_Buf[53+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
		
		convert_u16_data1 = (u16)(temp->ToUI3_Total_Voltage);        
		To_UI3_Buf[57] = convert_u16_data1>>8;/*相当于取高八位*/
		To_UI3_Buf[58] = convert_u16_data1;	 /*取低八位   转化为网络字节序*/
		
		To_UI3_Buf[59] = temp->ToUI3_SOC;   
		To_UI3_Buf[60] = temp->ToUI3_SOH;   
		
		convert_u32_data2 = htonl((temp->ToUI3_Sail_State));        /*转化为网络字节序*/
		for(cycle_number=0; cycle_number<4; cycle_number++)
			To_UI3_Buf[61+cycle_number] = *((u8 *)&convert_u32_data2 + cycle_number); 
		
		convert_u32_data2 = htonl((temp->ToUI3_Sys_Abnorm_Inf));        /*转化为网络字节序*/
		for(cycle_number=0; cycle_number<4; cycle_number++)
	 ...
	 if(1)/**/
	  {	
		...
		for (i=0; i<3; i++) 
		{
			...
			IMU_Prase_Data.Roll_Pitch_Yaw[i]=(IMU_Prase_Data.Roll_Pitch_Yaw[i]*(180/3.1415926));/*rad转换为°*/
			
			IMU_Prase_Data.Roll_Pitch_Yaw[2] = IMU_Prase_Data.Roll_Pitch_Yaw[2]+139.0;	/*软件校准用的*/
...
void Pack_Data_To_PSD(_ToPSD *temp)
{
	...
	if(1)/*整体控制,根据机器人执行后的条件，然后让频闪灯对应的灯亮起来*/
	{
		temp->head_buf = 0xAA;
		...
		temp->yellow_led_power=0x01; 
		   ...
		   to_PSD_buf[3] |= temp->white_led_power&0x01;	/*取white_led_power的最低位值的运算,white_led_power已经在powerprocess.c中被点亮赋值为0x01，其他灯类似*/
...
{ 
	ReadBIOSRealTime();
	Integ_t1=BIOS_RealTime.Hour*3600+BIOS_RealTime.Minute*60+BIOS_RealTime.Second;/*记录一次时间戳，单位秒*/	
	...
	/*将当前速度,时间覆盖上一量*/ 	
	Integ_t2=Integ_t1;/*将第一次时间戳保存在Integ_t2*/
