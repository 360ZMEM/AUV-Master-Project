...
char *g_ExtComName[] =
{
    ...
    "/com0/0","/com0/1","/com0/2","/com0/3",
    "/com0/4","/com0/5","/com0/6","/com0/7",
};
...
int lpt_test();
int speaker_test();
static void watch_dog(int para);
...
int eth_test()
{
    ...
    printf("Input net adapter number(0-5):\n");
    unit = GetKeyInput(10);
    unit %= 6;
    ...
    /* 两网卡不能设在同一网段 */
    /* 设置IP */
    sprintf(interfaceName, "fei%d", unit);
    sprintf(IpAddress, "192.168.%d.%d", unit+1, ip);
    ret = ifAddrSet(interfaceName, IpAddress);

    /* 使能网络接口 */
    ret = ifFlagSet(interfaceName, IFF_UP);
    if (ret != 0)
    {
    ...
    /* 设置子网掩码 */
    ret = ifMaskSet(interfaceName, 0xfffffff0);
...
int udp_server()
{
    ...
    printf("Input the net adapter number:(0-5:fei0-fei5 6-7:gei0-gei1):\n");
    input = GetKeyInput(10) % 8;
    if (input < 6)
    {
...
int tUdpServerRecv(int ip, int net)
{
    ...
    serverAddr.sin_addr.s_addr = ip;                /* 通过IP地址选择网卡 htonl(INADDR_ANY) */
...
int tUdpGroupRecv(int ip, int net)
{
    ...
    serverAddr.sin_addr.s_addr = INADDR_ANY;       /* 必须使用INADDR_ANY */
...
int tTcpServerRecv(int ip, int net)
{
    ...
    serverAddr.sin_addr.s_addr = htonl(INADDR_ANY); /* 通过IP地址选择网卡 htonl(INADDR_ANY) */
...
int tComRecv(int RxCom)
{
    ...
    ret += ioctl(fd, FIOBAUDRATE, COM_BAUD_RATE);   /* 波特率 */
...
int com_transmit()
{
	    ...
	    /* 指定发送串口 */
	    printf("Input Transmit COM Number:(1-8:COM 9-16:EXTCOM 17-24:CSDCOM) %dbps odd\n", COM_BAUD_RATE);
	    input = GetKeyInput(10);
	    TxCom = (input - 1) % 24;
	    ...
	    /* 配置串口参数 */
	    /* 数据位8位 停止位1位 奇校验 本地模式    CLOCAL | CS8 | PARODD | PARENB*/
	    ret += ioctl(fd, SIO_HW_OPTS_SET, CLOCAL |CS8 | PARODD | PARENB);
	    ret += ioctl(fd, FIOBAUDRATE, COM_BAUD_RATE);   /* 波特率 */
	    ret += ioctl(fd, FIOSETOPTIONS, OPT_RAW);       /* 原始模式OPT_RAW 行模式OPT_LINE 根据情况选择 */
	    ret += ioctl(fd, FIOFLUSH, 0);                  /* 清空缓冲区 */
...
int com_receive()
{
    ...
    /* 指定接收串口 */
    printf("Input Receive COM Number:(1-8:COM 9-16:EXTCOM 17-24:CSDCOM) %dbps odd\n", COM_BAUD_RATE);
    input = GetKeyInput(10);
    RxCom = (input - 1) % 24;
...
int std_test()
{
    ...
    else
    {
        ...
        /* 设置波特率 */
        ret |= ioctl (fd, FIOBAUDRATE, COM_BAUD_RATE);
...
int netswitch_test()
{
    ...
    ret = InitNetSwitch(master, slave, 0);
...
int soft_watchdog_test()
{
    ...
    /* 启动看门狗 */
    wdStart(s_wdId, s_ticks, (FUNCPTR)watch_dog, count);
...
static void watch_dog(int para)
{
    if (para > 0)
    {
        ...
        /* 重复触发看门狗 */
        wdStart(s_wdId, s_ticks, (FUNCPTR)watch_dog, para - 1);
    }
    else
    {
        logMsg("\nwatch_dog finished.\n", 0, 0, 0, 0, 0 ,0);
...
int tUdpBroadcastRecv(int ip, int net)
{
    ...
    for (sum = 0; s_task_over_flag != 1; sum += length)
    {
        ...
        clientAddr.sin_addr.s_addr = inet_addr(UDP_BROADCAST_ADDRESS);
...
int com_write_ex(unsigned short ushComBase, unsigned char TxBuffer[], int length)
{
    ...
    for (i = 0; i < length; i++)
    {
        ...
        while(!(sysInByte(ushComBase + 5) & 0x20)); /* 等待数据发送完成 */
    }
