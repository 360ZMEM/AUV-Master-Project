#!/usr/bin/env python3
"""Run PID terrain-following over multiple terrain seeds and aggregate results."""
...
def read_summary_statistics(path: Path) -> dict[str, float]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PID terrain multi-seed sweep.")
    parser.add_argument("--terrains", default="low,mid,high")
    parser.add_argument("--seeds", default="0,1,2")
    parser.add_argument("--duration", type=int, default=30)
    parser.add_argument("--label", default="terrain_pid_3seed")
...
def main() -> int:
    ...
    output_dir = args.output_root / f"terrain_pid_seed_sweep_{stamp}_{args.label}"
    ...
    for terrain in terrains:
        for seed in seeds:
            ...
            log_path = log_dir / f"{terrain}_seed{seed}.log"
            cmd = [
                "bash",
                str(RUNNER),
                str(args.duration),
            ...
            print(f"[terrain-seed] ({idx}/{total}) terrain={terrain} seed={seed}", flush=True)
            env = os.environ.copy()
            ...
            phase_dir = result_dir / "pid_terrain" if result_dir else None
            ...
            rows.append(row)
            print(f"[terrain-seed] finished terrain={terrain} seed={seed} status={status}", flush=True)
    ...
    for terrain in terrains:
        group = [row for row in rows if row["terrain"] == terrain]
    ...
    summary_fieldnames = ["terrain", "run_count", "ok_count"]
    for metric in METRIC_NAMES:
    ...
    lines = [
        "# Terrain PID Seed Sweep Aggregate Report",
        "",
        f"- Source: `{output_dir}`",
        ...
        "",
        "| terrain | ok/total | clearance RMSE to 3m | clearance mean | clearance std | clearance min | <1.5m ratio |",
        "|---|---:|---:|---:|---:|---:|---:|",
    ]
    for row in summary_rows:
        ok_total = f"{row['ok_count']}/{row['run_count']}"
        lines.append(
            "| {terrain} | {ok_total} | {rmse}±{rmse_std}m | {mean_v}±{mean_std}m | {std_v}±{std_std}m | {min_v}±{min_std}m | {viol}±{viol_std} |".format(
                terrain=row["terrain"],
                ok_total=ok_total,
                rmse=fmt(row["seabed_clearance_rmse_to_3m_mean"]),
                rmse_std=fmt(row["seabed_clearance_rmse_to_3m_std"]),
                mean_v=fmt(row["seabed_clearance_mean_m_mean"]),
                mean_std=fmt(row["seabed_clearance_mean_m_std"]),
                std_v=fmt(row["seabed_clearance_std_m_mean"]),
                std_std=fmt(row["seabed_clearance_std_m_std"]),
                min_v=fmt(row["seabed_clearance_min_m_mean"]),
                min_std=fmt(row["seabed_clearance_min_m_std"]),
                viol=fmt(row["seabed_clearance_safety_violation_ratio_1p5m_mean"]),
                viol_std=fmt(row["seabed_clearance_safety_violation_ratio_1p5m_std"]),
            )
        )
