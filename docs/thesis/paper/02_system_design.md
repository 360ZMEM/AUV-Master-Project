...
这四类约束之间并不是独立的，它们经常以"组合形式"出现：流场扰动会让 AUV 偏离规划路径，进一步使其在近底地形上接近危险距离；声呐可见性变化和磁场噪声同时发生时，状态估计的两个观测通道会同时退化。换句话说，系统设计必须假设"约束会叠加发生"，而不是按"每次只出现一种"乐观估计。这一认识直接决定了第 5 章联合应力工况场景的设计动机。
...
**图 2-1　AUV 系统自主功能闭环：环境与艇体、感知、状态理解、任务决策、运动控制、安全治理、人工监督和运行证据的协同关系** \label{fig:ch02-autonomy-loop}
...
**图 2-2　双脑异步硬件架构：Jetson 承担非实时感知、决策与优化，PC104/VxWorks 承担硬实时内环、驱动与失联保护，72 B 下行与 145 B 上行协议解耦两侧时序** \label{fig:ch02-dual-brain-async}
...
这五层之间最关键的一条边界，是"接口层"与"算法层"的解耦。算法层只关心"输入状态、输出控制"的纯数学映射，不关心数据来自仿真器还是真实硬件；接口层则把外部系统的协议细节、字节序、帧结构、健康字段等全部封装在内部。两层之间通过统一的状态数据结构与指令数据结构进行交互，不允许任何一方持有对方的具体类型，更不允许在算法层出现"如果是仿真器就走 A 分支、如果是真机就走 B 分支"这类条件分支。这种约束的直接收益是：仿真验证和实物部署共享同一套算法层与行为层代码，仅替换接口层实现。换句话说，论文第 5 章 PVS 仿真所验证的算法，与未来在真机上运行的算法是同一份代码——这是"仿真结论可外推"得以成立的工程前提。
...
**图 2-3　AUV 五层软件功能架构：入口配置、虚实适配、自治安全、数学算法、协议与物理约束及其接口传递** \label{fig:ch02-five-layer-architecture}
...
**图 2-4　算法仿真至全自主试验的验证部署阶梯：逐级能力释放、准入门与对应证据；右端为准入目标，不表示各阶段均已完成** \label{fig:ch02-verification-ladder}
...
其中 Pin12/13 承载 1PPS 硬件时钟同步：由 DVL/GPS 产生 1 Hz、脉宽 $10\ \mu s$ 的方波，接入声呐 `Trig1` 与 Jetson GPIO 中断，各节点在上升沿复位内部定时器，使跨设备绝对时间对齐误差 $< 10\ \mu s$。这一硬件级同步是第 3 章多源异步时间戳对齐算法的物理基础。
...
仿真阶段如何模拟这一隔离效果？显然 PVS 内置的物理模型并不会自动模拟"电源耦合干扰"，本系统通过感知引擎模块的两个机制间接体现：第一是磁饱和阈值——在磁畸变场景配置中设置阈值（1e-6 T 至 1e-8 T），把超出阈值的磁观测截断为饱和值，模拟"近场强干扰下传感器输出失真"；第二是磁场观测噪声注入——按可配置标准差向真值叠加高斯噪声，模拟隔离效果不理想时残留的随机噪声。这两个机制在算法层面足以验证 ES-EKF 对"磁观测降权"的鲁棒性；但隔离电源在实物上的真实效果（如纹波系数、共模抑制比、瞬态恢复时间），必须通过实测电源谱和磁传感器底噪谱才能给出量化结论。
...
**图 2-8　紧急状态切换与分层仲裁：通信、遥测、置信度、漏水和人工急停触发 ROS2 回零/锁定或 PC104 本地上浮/防触底，两类动作不属于同一处置层级** \label{fig:ch02-emergency-arbitration}

![任务完整生命周期](../figures/architecture/auv_v2_mission_lifecycle_flow.png)

**图 2-9　AUV 任务完整生命周期：操作员、安全守卫、决策控制脑和执行反馈端协同完成配置、授权、执行、回退、报告与人工回收** \label{fig:ch02-mission-lifecycle}
