...
**图 3-1　感知不确定性的跨层传递：观测退化经 ES-EKF 协方差与置信度映射形成共享信号，分别驱动行为降级和安全控制调度** \label{fig:ch03-uncertainty-highway}
...
第二种情形是"磁场主导，声呐沉默"。当电缆深埋（>1 m）或被沙泥完全覆盖时，声呐几乎无法分辨电缆，输出端的特征质量分数接近零，滤波器自动把 $\boldsymbol{R}_{\mathrm{sonar}}$ 放大到几乎屏蔽。此时磁场观测成为唯一的电缆相关线索——虽然几何精度不如声呐，但至少能给出"AUV 是否在电缆走向附近"的二值判断和粗略的横向偏移估计。
...
磁畸变（轻/重）场景通过专用配置生成，磁饱和阈值从 $10^{-6}$ T 收紧到 $10^{-8}$ T。该场景测试磁场观测不可靠时，状态估计是否能通过自适应模块降低磁观测权重，并维持估计连续性。其物理意图是模拟两类常见情形：一是 AUV 靠近母船等大磁源时的近场干扰，二是磁传感器自身量程受限导致的硬饱和。
...
总体上，当前已具备完整的状态估计算法框架、工具链和初步多种子仿真证据，能够支撑"ES-EKF 在多种传感器扰动下保持估计连续性"这一基本结论。但是，DVL 丢包长片段、磁畸变联合工况、硬件磁场标定和实测磁谱采集等系统性鲁棒性证据仍待补充。这些工作将在第 5 章实验补充计划和未来工作章节中明确列出。从论文写作的角度，本章应当被理解为"状态估计框架与初步验证"，而不是"完整的鲁棒性数据集"——这种分界既符合当前实验进度的真实情况，也为后续硬件实验留出了清晰的扩展接口。
