...
- 结合 `main.c` 新审计结果，`Work_Cmd_Execute(0x91)` 会清零 `Sys_Abnorm_Inf_Judgement` 和深度超限计数器；后续复验 BUG-4 时需要额外确认运行期 `work_cmd` 没有反复落到 `0x91`。
- 再结合 PySide6 源码审计，`main_window.py` 中 `set_work_instruct()` 会把 `self.work_instruct` 持续保持为 `0x91/0x92`，发送循环不会自动回落到 `0x00`；若 GUI 参与链路，需要把这类按钮视为“持续指令”而非“单次脉冲”。
...
## 十八、同步 origin 与 VxWorks 源码审计后的计划重整（2026-07-15）

### 18.1 仓库同步状态

本地 `main` 已与 `origin/main` 快进同步到同一提交：

```text
HEAD = origin/main = 075f8ee
```
...
- `.gitmodules`
- `docs/AGENTS.md`
- `docs/JETSON_DEPLOYMENT_CONTEXT.md`
- `docs/prompt.md`
- `README.md`
- `docs/INDEX.md`
- `docs_new/01_getting_started/02_quick_install.md`
- 若干脚本迁移到 `scripts/archive/`
...
该项为 submodule/外部依赖相关，本轮未处理，由用户后续手动处理。

### 18.2 VxWorks 源码只读审计结论
...
1. `Remote_Assignment(&Instruction_To_FMCU)` 会重新从 `UI_WIFI_Instruction` / `UI_LORA_Instruction` 读取 shadow 电机和舵角字段，并覆盖当前 `Instruction_To_FMCU`。
2. `Seafloor_Grounding_Arbitration()` 中 BUG-5/BUG-6 路径虽然先写了：
...
因此主推极可能被 UI shadow 中的 0 覆盖。

3. 这与实物 Telnet 证据一致：
   - 舵面进入 `Instruction_To_FMCU`
   - `motor1` 最终仍为 0

4. BUG-4 的深度保护路径在当前源码中已经看到“同步写 UI/LORA shadow 后再 `Remote_Assignment()`”的修复痕迹，但实机运行期仍未闭合，说明还需核对：
   - 当前烧录镜像是否对应这版源码
   - `Depth_Exceed_FromUI12_Depth_Para1` 是否真的累计到阈值
   - `Sys_Abnorm_Inf_Judgement` 的 Bit9 是否被后续路径清掉
   - `Remote_Assignment()` 之后是否又被别的模式流程覆盖

5. `DVL_Prase_Data` 结构含有 `u8[3]`、`int`、`float`、`bool` 混排，旧脚本中对 `BD_Height` / `BD_Check` 的经验偏移不能继续当真值使用。DVL execute 当前已证明“流程稳定”，但“偏移正确且注入值保持”仍未闭合。

### 18.3 当前未收口点重排

优先级最高，且最值得继续投入源码侧：

1. BUG-5/BUG-6：自救主推被 `Remote_Assignment()` 覆盖。
2. DVL：`BD_Height` / `BD_Check` 真实偏移与运行期保持。
3. BUG-4：Bit9 设置/保持与自救输出链。

仍可继续但暂不优先于源码问题的：

1. ROS2/fan-out 更长时间 soak。
2. 运行中 kill fan-out / kill bridge 的断链恢复。
3. sudo `PYTHONPATH` 透传下的 Zenoh 连续仲裁稳定性。
4. 仿真 `both` 和默认 stack 长窗口回归。

放到最后做同步验收的：

1. PySide6 GUI 手动并发 SOP。

### 18.4 下一步执行顺序

后续顺序调整为：

1. 先基于 `csd_vx6.8_lastest` 完成 VxWorks 关键定义核对。
2. 把 BUG-4 / BUG-5 / BUG-6 / DVL 偏移与运行期覆盖问题整理成更细的操作计划。
3. 继续做不依赖 GUI 的剩余项：
   - ROS2/fan-out 稳定性
   - Zenoh 仲裁稳定性
   - 仿真回归补齐
4. 最后再做 PySide6 GUI 同步验收。

详细计划已单独整理到：

- `.trae/documents/VxWorks未收口问题源码审计与后续调试计划_20260711.md`
- `.trae/documents/PC104实物fan-out与仿真共存架构分析_20260711.md`
