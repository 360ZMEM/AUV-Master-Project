...
PARAMS_FILE="${ROOT_DIR}/brain_linux/config/params.protocol_udp_pc104_fanout.yaml"

usage() {
  cat <<USAGE
Usage: $0 [--non-passive] [--passive] [--command-hz HZ] [--allow-nonzero-actuator] [--params-file PATH]

Default starts ROS2 bridge in passive mode.
Use --non-passive only for zero-actuator fan-out smoke/soak tests.
Use --allow-nonzero-actuator only for explicitly approved bench tests.
Use --params-file to select a non-default ROS2 bridge profile.
USAGE
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --allow-nonzero-actuator)
      ALLOW_NONZERO_ACTUATOR="true"
      shift
      ;;
    --params-file)
      PARAMS_FILE="${2:?missing value for --params-file}"
      shift 2
      ;;
    -h|--help)
      usage
...
if [[ ! -f "${PARAMS_FILE}" ]]; then
  echo "[start] params file not found: ${PARAMS_FILE}" >&2
  exit 1
fi
...
FANOUT_SAFETY_ARGS=()
if [[ "${ALLOW_NONZERO_ACTUATOR}" == "true" ]]; then
  echo "[start][WARN] non-zero actuator packets will be forwarded by fan-out."
  FANOUT_SAFETY_ARGS+=(--allow-nonzero-actuator)
fi
