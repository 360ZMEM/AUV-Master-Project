...
要把上述定性逻辑落到工程可实现的判据上，还需要给出两类通道的失效边界与量化触发条件。声呐侧的失效边界由其物理机理决定：侧扫声呐（540/850 kHz）在电缆裸露或位于开阔沟槽时成像清晰，但一旦掩埋深度超过约 10 cm，其对电缆的识别率骤降至接近 0\%，浅水区还会因多径产生伪影；浅地层剖面仪（4--24 kHz）虽能穿透泥下 0.5--3 m，但只有在 AUV 近乎正上方飞过时才准确，横向漂移超过约 1 m 即立即失锁，且无法用于大面积搜索。磁场侧则存在另一类边界，当电缆曲率半径 \(R < 200\text{ m}\)（弯道）时，单磁力计观测到的 \(|B|\) 会骤降，但仅凭标量幅值无法判断电缆是左拐、右拐还是埋深突然变大，若不主动横向机动就必然产生航向误判。

据此，本系统把接力过程组织为一个由自适应协方差 \(\boldsymbol{R}_k = \text{diag}(R_{mag}, R_{sonar})\) 驱动的决策流程，覆盖三类典型工况：工况 A（裸露、水质好）声呐质量分数 \(S_{sonar} \approx 1.0\)，取 \(R_{sonar}\) 小、\(R_{mag}\) 中，以声呐为主实现厘米级跟踪；工况 B（泥下掩埋）\(S_{sonar} \to 0\)、\(R_{sonar} \to \infty\)、\(R_{mag}\) 收小，主导权接力给磁力计；工况 C（弯道 \(R < 200\text{ m}\)）状态协方差 \(\boldsymbol{P}_k\) 因方向模糊而增大，触发行为树执行 \(15^\circ\) 小角度之字形横切，用第 2.3.5 节的 FWHM 定理重新解算方向与深度。这一流程把``何时机动''从随机搜索变成由协方差判据触发的确定性动作。

在此基础上可以刻画系统的两条可靠性底线。其一是稀疏声呐鲁棒性：即便声呐有效率仅约 20\%（等效丢包 80\%、刷新率降至 0.4 Hz），在弯道 \(R < 200\text{ m}\) 处仍可依靠磁场包络加 DVL 递推撑过盲区，把跟踪误差维持在 \(\le 0.2\text{ m}\)。其二是全盲自救：当声呐 100\% 失效且弯道处磁场丢锁时，\(\boldsymbol{P}_k\) 超过卡方门限会触发行为树主动之字形重捕，避免系统在感知全失时盲目前进。这两条底线也直接定义了第 5 章接力消融实验的三个对照组，声呐 100\%（理想）、声呐 20\%（主推，验证严重丢包下仍可跟踪）、声呐 0\%（退化，验证纯磁直道可跑、弯道触发行为树重捕）。
