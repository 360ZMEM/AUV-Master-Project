...
其中 \(h_{alt}\) 为高度计读数。这一反演的关键优势在于它\textbf{完全解耦了对未知电流 \(I\)、铠装衰减率 \(\eta\) 与磁偶极矩 \(m_{eq}\) 的依赖}，这些正是海况下最难先验标定的量。据此，磁学观测向 ES-EKF 提供两个绝对约束：横向中心线位置（峰值时刻 \(x = 0\)）与垂直距离 \(z_{mag}\)。前者修正横向偏移 \(d_{lat}\)，后者作为埋深观测 \(z_{mag}\) 并入第~\ref{sec:ch03-3-3-1}节的观测向量。相应的观测方程与雅可比为
...
轻、重两档磁畸变以及综合压力场景均已纳入三种子结果。该组实验验证的是磁观测降权时的估计连续性；真实频谱、相位畸变和软饱和将由实测噪声回放进一步补充。连同上一小节的 DVL 丢包四档与声呐杂波，本章鲁棒性实验共覆盖八类扰动场景（DVL 四档、磁畸变两档、声呐杂波与综合压力），另设干净基线作对照；各场景的名义强度与用途见附录~\ref{sec:app-a-1}。
