...
本章的目的是在纯仿真与数字孪生条件下，围绕第 1.4 节确立的``声磁协同状态估计、感知不确定性驱动控制、行为树安全决策''三条主线，通过多场景、多随机种子、多失效模式与多控制器消融实验，验证所提方法在设定扰动条件下的相对有效性并界定其适用边界。为使每条结论都能追溯到具体实验条件，本章的评价体系沿四个维度组织：其一是仿真任务定义，即海缆自主巡检从搜索、捕获到沿线跟踪的闭环任务与终止判据（第~\ref{sec:ch05-5-1-2}节、第~\ref{sec:ch05-5-2}节）；其二是场景、传感器与扰动建模，即海流、传感器退化、通信故障与磁/声观测噪声的可控注入（第~\ref{sec:ch05-5-2}节、第~\ref{sec:ch05-5-2-3}节）；其三是对比方法与消融设置，即固定权重与不确定性感知控制、单模态与声磁融合、行为树与状态机在同源同种子下的成对比较（第~\ref{sec:ch05-5-1-3}节、第~\ref{sec:ch05-5-5}节）；其四是评价指标，即定位、控制、地形跟随与巡检四类可复现量化指标（第~\ref{sec:ch05-5-1-4}节）。本章所有统计结论均限定于仿真口径，其证据强度与向真机迁移的剩余验证集中于第~\ref{sec:ch05-5-5-9}节与第~\ref{sec:ch05-5-6}节讨论。
...
前两节交代了仿真实施架构、运行生命周期、评价口径与场景库，本节起进入全文的核心证据部分。为避免把分散的单点实验平铺成流水账，本章把仿真证据归并为三条相互独立、各自封闭的主线依次展开：状态估计仿真（本节）、控制与安全仿真（第~\ref{sec:ch05-5-ctrl}节）、端到端海缆巡检数字孪生仿真（第~\ref{sec:ch05-e2e}节）。每条主线只保留主结果表、结论叙述与一句指向证据边界总表（第~\ref{sec:ch05-5-6}节）的限定，过程性与分源诊断细节收进主表或随附录归档；面向实物的硬件与实验室证据及迁移路径另集中于第~\ref{sec:ch05-5-3}节。这一组织延续第 4 章的鲁棒性分析口径：任何结果在写入论文时都显式标注样本量、测量边界与可外推条件，使读者能清楚看到每条结论各自的证据强度，避免``用单次基准代替多种子统计''的过度推断。
...
噪声回放对照实验的均值对照图随附录归档，正文以表~\ref{tab:ch05-r19-noise-replay} 为准。
...
滤波策略对照实验的策略对照图随附录归档，正文以表~\ref{tab:ch05-r20-filter-strategy} 和上述机理归因为准。
\FloatBarrier
...
对应三档消融图随附录归档，正文以表~\ref{tab:ch05-terrain-ablation} 为准。
\FloatBarrier
...
分源置信正式矩阵的主要性能结果见表~\ref{tab:ch05-r13-v2-main-results}。为突出实验结论而不是实现过程，表中只报告横向 RMSE、控制变化率、安全违规和授权模式四类结果。可以看到，保守 UA 并未系统改善横向 RMSE；全局均值由固定权重的 7.932 m 变为 8.006 m。但它显著降低了多类高压力场景的控制变化率，全局 RMS 由 0.247 降为 0.085，其中发卡、横流和综合极端分别降低约 89.8\%、52.0\% 和 84.4\%。因此，本轮矩阵支持的主结论是``分源置信下的保守控制平滑与实时可解性''，而不是``路径跟踪精度全面提升''。
...
\subcaptionbox{保守 UA 对控制变化率的影响\label{fig:ch05-r13-v2-control-rate}}{%
  \includegraphics[width=0.86\linewidth,height=0.27\textheight,keepaspectratio,alt={六类代理场景中固定权重与保守不确定性感知控制的控制变化率成对对比；发卡、横流和综合极端显著下降}]{experiments/control/r13_v2_control_rate_paired.pdf}}\\[2pt]
\subcaptionbox{分源置信下的授权模式分布\label{fig:ch05-r13-v2-authority}}{%
  \includegraphics[width=0.86\linewidth,height=0.27\textheight,keepaspectratio,alt={保守不确定性感知控制下六类代理场景的保持、搜索、跟踪授权占比堆叠图}]{experiments/control/r13_v2_authority_modes_ua.pdf}}\\[2pt]
\subcaptionbox{分源置信 \(p_{\mathrm{track}}\) 分布\label{fig:ch05-r13-v2-ptrack}}{%
  \includegraphics[width=0.86\linewidth,height=0.27\textheight,keepaspectratio,alt={保守不确定性感知控制下六类代理场景声呐 p_track 分布箱线图；埋设间断动态范围最大，综合极端未达到跟踪进入门}]{experiments/control/r13_v2_p_track_boxplot_ua.pdf}}
...
原生场景因子实验闭合的是``因子可独立操纵、真值与量测不串扰''这一生成器契约，而非控制性能结论；上表已给出四个因子开关的可观测差异，噪声时间结构与横流时序波形随附录 A.7 归档。原生因子闭环实验在此基础上把原生生成器接入制导级 MPC、分源跟踪授权和保守质量控制策略，形成不调用 PVS 原生定深定向自驾的全因子离线闭环矩阵，用于排除原生声磁、地形与横流因子无法进入控制闭环的接口风险，并量化主效应与二阶交互。
...
\textbf{证据边界（必须与上述结论同时引用）}：本节验收链路的口径限制已并入第~\ref{sec:ch05-5-6}节 的证据边界总表——核心是近零路由偏移与近常值置信度只证明链路与评分逻辑正确而非真实检测精度，且就绪判定限定于有效巡检窗口、三次运行为同一场景连续重复，其余分项不再重复展开。因此，本节可写入论文的结论是：``声磁电缆巡检的运行时数据到离线行业标准评分到工业验收到多次运行聚合的全链路已闭环，在数字孪生确定性先验下三次运行全部就绪且通过、初步验收就绪''；不可越界写成``通过真实海缆检测精度验收''或``通过现场行业标准验收''。
...
其次，闭环恢复在六自由度闭环中被复现，与开环冻结形成直接对照。重档车辆起始横偏约 −10.25 m（由先验畸变注入），在线修正被接受后横偏在约 12 秒内收敛进约 ±3.4 m 的验收廊道并全程保持（末段带符号横偏收敛到约零），全程逐帧的航向修正超过 1° 的占 1112/1219 帧，即车辆被真实重新操舵去贴合修正后的先验，而非此前``横偏被冻结在开环几何差上单调累积''。作为同源对照，本环同时跑了在线修正关闭的六次基线运行：中、重两档全程最大路由偏移分别约 15.3/20.1 m、窗口内点数为零、三次全部无效，与失效边界闭环时逐位重合。开关对照下的横偏恢复轨迹与在线修正接受时序见图~\ref{fig:ch05-pvs-closedloop-recovery}（同批次二维俯视回放为 \(n=1\) 科普性插图，不在正文展开，随附录 A.9/Artifact Manifest 归档）：
...
本轮从配置、仿真采样、模拟标定到估计配置应用形成完整闭环，平移误差由 0.1086 m 降至 0.0041 m、安装角误差由 1.5000° 降至 0.0704°，说明杆臂改正链路与文件契约能够承载标定结果。该组数值来自仿真外参标定，用于验证方法和软件闭环；真机九参数与安装外参的计量标定作为剩余计量验证项，与本章其余边界一并归纳。
...
上述四类迁移差距并非本文方法的失效项，而是分层验证阶梯有意留待硬件在环与实验室层解决的刻度：磁观测链路噪声、等效通电电缆磁场反演与载荷舱磁卫生构成后续实物证据链中三个各自独立、可交付的单元（其推进路径见第~\ref{sec:ch06-6-3}节），可在不改动上层算法接口的前提下逐层接入，而不构成对当前仿真结论的否定。

\section{证据边界、负结果归因与剩余验证}\label{sec:ch05-5-6}

本节把全章实验的负结果、口径限制和不可外推项集中收束为六个问题域：磁探测计量与验收口径、ES-EKF 统计一致性、在线先验与几何前提、不确定性感知控制证据、CBF 与原生因子闭环、实物通信与端到端时延。各域的边界归因、已采取处置与可写结论、剩余验证逐条汇总于附录表~\ref{tab:ch05-negative-results}；第~\ref{sec:ch05-5-5}节、第~\ref{sec:ch05-5-ctrl}节、第~\ref{sec:ch05-e2e}节 各结果主线只保留正向主结果与最小必要限定，相同局限统一指向该表。需要强调的是，该表``剩余验证''列并非未知缺口清单，而是已按可独立交付的实物证据链拆解的推进路径——每项均可在当前仿真结论之上增量验证，其向实物层推进的路线见第~\ref{sec:ch06-6-3}节，不构成对已有结论的否定。
