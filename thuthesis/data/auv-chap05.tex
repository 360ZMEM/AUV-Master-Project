...
真机侧时序口径由 PC104 UDP 探针补充：主机按 10 Hz 发送安全零推力 \texttt{\$CKTH} 下行包，并记录 \texttt{\$AUV} 上行到达间隔、帧号缺口、解析错误与 PC104 运行时长单调性。30 s 主机中继记录中，300 帧零推力下行进入中继，450 帧上行被容器解析，解析错误为 0；主上行到达频率约 15.000 Hz，到达间隔 p50/p95/p99 为 57.983/85.710/85.944 ms。该探针只能报告到达间隔与抖动，不构成单向物理延迟，边界见 \S\ref{sec:ch05-5-6}。
...
Jetson 与 PC104/VxWorks 之间的二进制通信链路需要在实物环境下核验端口、帧序与失联保护。该链路的下行帧携带工作模式、任务指令、舵角、推力与一组可调浮点参数，上行帧携带姿态、卫星定位、三轴速度、深度、电压、温度、漏水与故障位图（学术数据契约见附录 A.6）。在上位机端与 VxWorks 端的独立开发过程中已发现若干处协议偏差，包括帧头长度、压力与航向字段的错位、深度缩放因子的不一致、舵角类型与符号约定的差异等，这些偏差已在仿真期通过协议单元测试闭环，实物部署时仍需逐一复核。当前 30 s host-relay 零推力探针已观测到 450 帧可解析上行、0 次解析错误和 0 个大于 1 的前向跨帧；但在缺少固件回显或可信共享时钟时，该探针只能报告 Jetson 侧到达间隔，不能报告单向物理时延或往返时延。

\subsection{Jetson Orin NX 在线求解实时性实测}\label{sec:ch05-jetson-realtime}

为把目标平台算力证据与网络到达时序、完整 ROS2 链路时延区分开，本文在 Jetson Orin NX 的 25 W、8 核模式下，对当前 \(N=20\)、\(\Delta t=0.2\text{ s}\) 的 MPC 求解入口实施独立微基准。测试对应仓库提交 \texttt{8419691}；正式运行前记录 5 s 空闲基线，预检未发现软件安装、编译或桌面监控等已知竞争进程。稳态 cold/warm 各运行 200 次，\(8\text{ m}\) 初始深度到 \(3\text{ m}\) 目标深度的约束压力档各运行 50 次。表~\ref{tab:ch05-jetson-realtime} 给出 wall-clock 统计；PC104 行来自独立的 30 s host-relay 原始记录，其数值类型明确为上行到达间隔。

\begin{table}[htbp]
\centering
\scriptsize
\setlength{\tabcolsep}{2.7pt}
\caption{Jetson Orin NX 求解器微基准与 PC104 上行到达间隔}\label{tab:ch05-jetson-realtime}
\resizebox{\linewidth}{!}{%
...
\midrule
PC104 上行 & host-relay & 449 & 66.624 & 57.983 & 85.710 & 85.939 & 86.290 & -- & 到达间隔，非单向/RTT \\
\bottomrule
\end{tabular}}
\end{table}
...
同一短时 clean run 共记录 60 个整机资源样本，覆盖 30.224 s；CPU mean/p95/max 为 39.9\%/60.0\%/75.1\%，内存 mean/max 为 4838.2/4918.6 MiB，最高温度为 57.4\textdegree C，0.5 s 采样间隔的 p95 为 0.505 s。CPU 分阶段均值为 idle/steady/stress=34.3\%/42.6\%/38.3\%。这些 CPU/GPU、内存和温度是 Jetson 整机遥测，不是求解器进程独占归因；空闲段本身仍含桌面负载，且 30 s 量级测试不能替代 30 min 全栈热稳定实验。

通信侧 449 个有效上行到达间隔的 p50/p95/p99 为 57.983/85.710/85.944 ms。帧号审计中的 150 次零增量是重复帧号，而非 150 帧丢失；大于 1 的前向跨帧为 0、估算丢帧为 0。求解 wall time 与该到达间隔来自不同链路和时钟口径，二者不得相加为 ADC--EKF--行为树--控制--PC104 的端到端时延。当前结果关闭的是目标平台求解器的短时可运行性与复现入口，不关闭完整 ROS2 调度、长时热稳定或 Jetson--PC104 单向物理时延；这些边界集中收束于 §\ref{sec:ch05-5-6}，后续刻度见 §\ref{sec:ch06-6-3}。
...
综合这六个问题域可见，当前方法的主要算法链已经闭合，Jetson 目标平台的独立求解器短时 clean run 也已给出稳态周期余量；剩余工作集中在统计标定、分源置信映射、原生耦合闭环、ADC--EKF--行为树--控制--PC104 全栈时序、长时热稳定和系统级计量验证。它们限定结论的适用范围，但不改变本文关于低成本窄带采集、声磁协同估计、不确定性感知控制和闭环恢复机制的核心论证。
...
三条仿真主线之外，低成本磁采集、SK2301 窄带等效分辨率、45 Hz 复 I/Q 联合反演、磁外参标定链路、PC104 时序探针和双脑通信逻辑为实物部署提供了物理锚点。它们当前支撑的是``关键链路可实现、接口可复用、部署路径可分阶段推进''，而不是完整水下自主巡检已经闭合。全章因此形成的结论边界是清晰的：算法链路已在可重复仿真与半实物口径下闭合，实物层还需沿磁采集链、等效海缆扫描、磁卫生、Jetson 实时性、声呐接口替代和 PC104/VxWorks 接入逐级补强。统计、场景、接口和计量边界已集中于 §\ref{sec:ch05-5-6} 与附录表~\ref{tab:ch05-negative-results}，不在本章小结重复展开。
