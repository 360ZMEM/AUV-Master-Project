...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio,alt={三维海底曲面与 PID 地形跟随轨迹：确定性重建的起伏海床曲面覆盖 AUV 全轨迹，蓝色轨迹在海床上方以近似恒定净空推进，橙色散点为沿程 DVL 实测海底验证锚，重建与实测沿程相关系数约 0.91}]{terrain_following/terrain_3d_pid_terrain_trajectory.pdf}
\caption{三维海底曲面与 PID 地形跟随轨迹（确定性地形重建 + 沿程 DVL 实测海底验证锚，\(n=1\)）：蓝色为 AUV 轨迹，绿色虚线为 3~m 目标净空对应的目标深度路径，曲面颜色表示向下为正的海底深度，橙色散点为 DVL 实测海底高度，重建与实测沿程相关系数约 0.91}\label{fig:ch05-terrain-3d}
\end{figure}
\FloatBarrier
...
\subcaptionbox{保守 UA 对控制变化率的影响\label{fig:ch05-r13-v2-control-rate}}{%
  \includegraphics[width=0.86\linewidth,height=0.27\textheight,keepaspectratio,alt={R13-v2 六类代理场景中固定权重与 conservative UA 的控制变化率成对对比；发卡、横流和综合极端显著下降}]{experiments/control/r13_v2_control_rate_paired.pdf}}\\[2pt]
\subcaptionbox{分源置信下的授权模式分布\label{fig:ch05-r13-v2-authority}}{%
  \includegraphics[width=0.86\linewidth,height=0.27\textheight,keepaspectratio,alt={R13-v2 UA 模式下六类代理场景的 HOLD、SEARCH、TRACK 授权占比堆叠图}]{experiments/control/r13_v2_authority_modes_ua.pdf}}\\[2pt]
\subcaptionbox{分源置信 \(p_{\mathrm{track}}\) 分布\label{fig:ch05-r13-v2-ptrack}}{%
  \includegraphics[width=0.86\linewidth,height=0.27\textheight,keepaspectratio,alt={R13-v2 UA 模式下六类代理场景 sonar p_track 分布箱线图；埋设间断动态范围最大，综合极端未达到 TRACK 进入门}]{experiments/control/r13_v2_p_track_boxplot_ua.pdf}}
