...
这四类约束之间并不是独立的，它们经常以``组合形式''出现：流场扰动会让 AUV 偏离规划路径，进一步使其在近底地形上接近危险距离；声呐可见性变化和磁场噪声同时发生时，状态估计的两个观测通道会同时退化。换句话说，系统设计必须假设``约束会叠加发生''，而不是按``每次只出现一种''乐观估计。这一认识直接决定了第 5 章 \texttt{combined\_\allowbreak{}stress} 联合工况场景的设计动机。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio,alt={AUV 系统自主功能闭环（任务执行、授权监控、急停与失联保护的安全链路）}]{architecture/auv_system_autonomy_functional_loop.png}
\caption{AUV 系统自主功能闭环（任务执行、授权监控、急停与失联保护的安全链路）}\label{fig:ch02-autonomy-loop}
\end{figure}
\FloatBarrier
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio,alt={多源异构双脑异步硬件架构（Jetson 感知决策脑、AMD/VxWorks 安全执行脑及失联保护）}]{architecture/auv_v2_dual_brain_async_hardware.png}
\caption{多源异构双脑异步硬件架构（Jetson 感知决策脑、AMD/VxWorks 安全执行脑及失联保护）}\label{fig:ch02-dual-brain-async}
\end{figure}
\FloatBarrier
...
\bottomrule\noalign{}
\endlastfoot
应用层 & \texttt{apps/} & 入口脚本，初始化配置、组装依赖、启动主循环 \\
接口层 & \texttt{interfaces/} & 对外部系统（仿真器、通信中间件、硬件）的适配封装 \\
行为层 & \texttt{behavior/} & 安全守卫、状态机、命令过滤等运行时行为管理 \\
算法层 & \texttt{algorithm/} & 核心控制/导航/估计算法的纯计算实现 \\
协议层 & \texttt{common/} & 数据结构定义、枚举、物理常量、环境工具等公共基础设施 \\
\end{longtable}

这五层之间最关键的一条边界，是``接口层''与``算法层''的解耦。算法层只关心``输入状态、输出控制''的纯数学映射，不关心数据来自仿真器还是真实硬件；接口层则把外部系统的协议细节、字节序、帧结构、健康字段等全部封装在内部。两层之间通过纯数据结构（\texttt{AUVStateData}、\texttt{AUVCommand} 等）进行交互，不允许任何一方持有对方的具体类型，更不允许在算法层出现``如果是仿真器就走 A 分支、如果是真机就走 B 分支''这类条件分支。这种约束的直接收益是：仿真验证和实物部署共享同一套算法层与行为层代码，仅替换接口层实现。换句话说，论文第 5 章 PVS 仿真所验证的算法，与未来在真机上运行的算法是同一份代码，这是``仿真结论可外推''得以成立的工程前提。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio,alt={AUV 五层软件功能架构（应用、接口、行为、算法与协议层的职责和接口）}]{architecture/auv_v2_five_layer_functional_architecture.png}
\caption{AUV 五层软件功能架构（应用、接口、行为、算法与协议层的职责和接口）}\label{fig:ch02-five-layer-architecture}
\end{figure}
\FloatBarrier
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio,alt={从数字孪生到实物部署的验证阶梯（PVS、HoloOcean、Mock AMD 与真实 AUV 的证据边界）}]{architecture/auv_system_verification_deployment_ladder.png}
\caption{从数字孪生到实物部署的验证阶梯（PVS、HoloOcean、Mock AMD 与真实 AUV 的证据边界）}\label{fig:ch02-verification-ladder}
\end{figure}
\FloatBarrier
...
单芯电缆是理解海缆磁场的最基础情形。把电缆近似为无限长直导线，由毕奥-萨伐尔定律（Biot-Savart Law）\cite{griffiths2017electrodynamics}沿导线积分，可得径向距离 \texttt{r} 处的磁感应强度解析解：
...
时，双相缩比电缆产生的远场磁场分布形态与真实三相海缆\textbf{数学同构}，为第 5 章的缩比台架验证提供了理论依据。
...
本文进一步以 45 Hz 可控电流、三轴 TMR 与 ArUco 位姿构成联合实验，对直导线模型进行空间复核。共获得 777 个时间对齐观测点；保留三轴幅值、方向与相位的复 I/Q 拟合在自由复尺度下达到 \(R^2=0.8792\)，在固定峰值电流 \(2.6140\text{ A}\) 下达到 \(R^2=0.8711\)。该结果表明，移动单三轴传感器与外部位姿能够形成合成空间阵列，并从连续轨迹中恢复电缆方向、相对位置与同频场分布。实验采用无限长直导线和公共复尺度，因而在本章中用于验证几何建模与可观测性，不把它扩展解释为多芯铠装海缆的绝对埋深标定；对应实测结果见第 5.4 节。
...
具体而言，给定 AUV 当前水深 \texttt{h} 和图像中某像素对应的斜距 \texttt{r\_s}，目标在海底的水平距离 \texttt{r\_g} 满足 \texttt{r\_\allowbreak{}g\ =\allowbreak{}\ sqrt(r\_\allowbreak{}s²\ -\ h²)}。这意味着图像中``看起来等间距''的像素，在海底实际上并非等间距，靠近 AUV 正下方的像素被压缩，远离 AUV 的像素被拉伸。如果状态估计器直接用斜距像素坐标作为观测，会引入系统性的几何偏差。声速剖面（SVP）变化和多径反射会进一步引入二阶误差，这些误差在浅水或声速突变区尤其明显。

需要说明的是，本文只使用上述斜距到地距的一阶几何关系（\texttt{r\_\allowbreak{}g\ =\allowbreak{}\ sqrt(r\_\allowbreak{}s²\ -\ h²)}）作为声呐观测映射的物理前提，用于说明像素坐标不能直接当作海底真实坐标。声速剖面变化、多径反射和真实声呐成像畸变属于二阶误差，本文不将其纳入计量结论，也不在台架或海试前对声呐几何精度作定量宣称。
...
这三类标定中，``软铁/硬磁干扰''最容易被忽略却影响最深。AUV 本体中的铁磁材料会被外部地磁场极化（软铁效应），同时其自身的永磁结构会叠加一个固定偏置（硬磁效应）。两者共同作用使磁传感器观测到的磁场与真实环境磁场之间存在一个``\texttt{B\_\allowbreak{}obs\ =\allowbreak{}\ M·B\_\allowbreak{}env\ +\ b}''形式的线性畸变，其中 \texttt{M} 是 3×3 软铁矩阵、\texttt{b} 是 3×1 硬磁偏置，共 9 个参数。九参数在线标定\cite{cui2018magnetometer,vasconcelos2011geometric}通过让 AUV 在多个航向上做圆周运动，从观测的椭球拟合反推 \texttt{M} 和 \texttt{b}，再据此对原始磁观测做反向修正。这是磁场观测能用于状态估计的硬性前置工作，论文写作时必须明确标注其完成状态。
...
其中 Pin12/13 承载 1PPS 硬件时钟同步：由 DVL/GPS 产生 1 Hz、脉宽 \(10\ \mu s\) 的方波，接入声呐 \texttt{Trig1} 与 Jetson GPIO 中断，各节点在上升沿复位内部定时器，使跨设备绝对时间对齐误差 \(< 10\ \mu s\)。这一硬件级同步是第 3.2.1 节多源异步时间戳对齐算法的物理基础。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio,alt={编号 6\# TMR8637 模组三轴测试汇总与电压-磁场传输曲线（随附纸质测试报告）}]{hardware/tmr8637_test_report/tmr8637_axis_calibration_report.jpg}
\caption{编号 6\# TMR8637 模组三轴测试汇总与电压-磁场传输曲线（随附纸质测试报告）}\label{fig:ch02-tmr8637-axis-report}
\end{figure}
\FloatBarrier

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio,alt={编号 6\# TMR8637 模组三轴电压噪声与磁场噪声谱密度曲线（50 Hz 处约为 20--30 pT/\textbackslash sqrt\{\textbackslash mathrm\{Hz\}\}）}]{hardware/tmr8637_test_report/tmr8637_noise_psd_report.jpg}
\caption{编号 6\# TMR8637 模组三轴电压噪声与磁场噪声谱密度曲线（50 Hz 处约为 20--30 pT/\(\sqrt{\mathrm{Hz}}\)）}\label{fig:ch02-tmr8637-noise-psd}
\end{figure}
\FloatBarrier
...
仿真阶段如何模拟这一隔离效果？显然 PVS 内置的物理模型并不会自动模拟``电源耦合干扰''，本系统通过 \texttt{perception\_\allowbreak{}engine.\allowbreak{}py} 的两个机制间接体现：第一是磁饱和阈值，在 {\small \texttt{scenario\_\allowbreak{}mag\_\allowbreak{}distortion\_\allowbreak{}*.\allowbreak{}yaml}} 中设置阈值（1e-6 T 至 1e-8 T），把超出阈值的磁观测截断为饱和值，模拟``近场强干扰下传感器输出失真''；第二是磁场观测噪声注入，按可配置标准差向真值叠加高斯噪声，模拟隔离效果不理想时残留的随机噪声。这两个机制在算法层面足以验证 ES-EKF 对``磁观测降权''的鲁棒性；但隔离电源在实物上的真实效果（如纹波系数、共模抑制比、瞬态恢复时间），必须通过实测电源谱和磁传感器底噪谱才能给出量化结论。
...
为什么 ESTOP 模式必须走``UDP + Zenoh 双通道''？答案直指安全冗余的本质，任何单一通信链路都可能因软件崩溃、网络拥塞或硬件故障而中断，急停作为``最后一道安全闸门''承担不起任何单点失效。具体实现上，UDP 通道通过周期发送 \texttt{Work\_\allowbreak{}Cmd=\allowbreak{}0x02\ +\ Motor\_\allowbreak{}Speed=\allowbreak{}0\ +\ ctrl\_\allowbreak{}mode=\allowbreak{}0x01} 的急停帧（20 Hz）直达 AMD/VxWorks，仅依赖最底层的 \texttt{common.protocol} 模块；Zenoh 通道则向 Jetson 上的 ROS2 节点发送任务取消信号，让上层行为树主动放弃当前任务。两条通道走完全独立的代码路径，确保即使上层 ROS2/Zenoh 协议栈整体崩溃，UDP 急停帧仍能通过用户态最小依赖路径到达 AMD。
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio,alt={紧急状态切换与仲裁（四类安全守卫的触发优先级及目标安全状态）}]{architecture/auv_v2_emergency_transition.png}
\caption{紧急状态切换与仲裁（四类安全守卫的触发优先级及目标安全状态）}\label{fig:ch02-emergency-arbitration}
\end{figure}
\FloatBarrier

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio,alt={AUV 任务完整生命周期（操作员、仲裁器、决策脑与执行脑的协同流程）}]{architecture/auv_v2_mission_lifecycle_flow.png}
\caption{AUV 任务完整生命周期（操作员、仲裁器、决策脑与执行脑的协同流程）}\label{fig:ch02-mission-lifecycle}
\end{figure}
\FloatBarrier
