...
这句原则展开为四道防线：第一道是行为树 failsafe 节点（在任务调度层，将异常作为高优先级事件抢占正常任务）；第二道是安全仲裁器（在控制输出端，把任何越界指令钳制到安全包络内）；第三道是上位机 ESTOP 分层下发（向 PC104 发送回遥控、零执行器的 UDP 急停帧，并在 Zenoh 可用时把同一原始帧镜像给 Jetson）；第四道是 VxWorks 失联保护（执行层自身在长时间未收到上行指令时主动停机）。这四道防线在功能上形成冗余，但各自覆盖的故障域和当前验证深度并不相同。系统自主功能闭环如图~\ref{fig:ch02-autonomy-loop}所示：
...
确立了三类硬性约束之后，下一步要回答的问题是：用什么样的硬件骨架来承载它？这一节先讨论架构层面的``宏观选型''，再在 2.2.2 节给出软件层面的``微观分层''。本系统采用``执行层 + 感知决策层''的异步双脑架构，执行层由实物部署阶段的 AMD Geode LX 800 实时控制板（或等价实时控制端）承担，负责底层舵机驱动、推进器控制、遥测采集和失联保护；感知决策层由 Jetson/ROS2 承担，负责状态估计、行为树任务调度、路径生成和控制参考计算。

从硬件配置看，两个脑的定位差异是刻意为之的：安全执行脑采用 AMD Geode LX 800（500 MHz 主频、256 MB 内存、VxWorks 6.8 RTOS；下文按其 PC104 板型规格简称 PC104），其算力不足以毫秒级完成 MPC 浮点矩阵求逆或图像卷积，但换来的是微秒级中断响应和硬实时确定性，专司主/侧推进器 PWM、CAN、舵机驱动与舱压/温度/漏水轮询；感知决策脑采用 NVIDIA Jetson Orin NX（16 GB、100 TOPS、ROS2），承担 FFT、YOLO、EKF 与 UA-MPC 等高算力非实时任务。两脑之间以 UDP 报文通信（决策脑下发 \texttt{\$CKTH} 帧、执行脑上传 \texttt{\$AUV} 帧，详见 2.5.1 节），安全响应采用分层看门狗：ROS2 仲裁层对 PC 原始命令设置 1.0 s 软告警和 1.5 s hard ESTOP，对 MPC 命令设置 0.5 s 超时回退；PC104 执行层则以 0.1 s 周期计数检查 Jetson 模式下的有效指令，约 1.0 s 超时后切回遥控模式、清零推进并上报告警。深度越限、近底或 DVL 失锁等航行安全风险由 PC104 本地逻辑进一步覆盖为上浮或防触底动作。这一``配置低但确定性高''的执行脑设计，正是把``安全边界''从非实时 Linux 进程中剥离出来的物理保证。
...
\label{eq:ch02-static-field}
\end{equation}

其中 \(\mu_0\) 为真空磁导率，\(\mu_r\) 为介质相对磁导率（海水与海床沙石 \(\mu_r \approx 1\)），\(I_{dc}\) 为导线电流，\(\hat{\boldsymbol{\theta}}\) 为绕导线的切向单位矢量。式\eqref{eq:ch02-static-field}表明直流单芯电缆磁场沿 \(1/r\) 规律衰减。对单相交流电缆，电流 \(i(t) = I_{ac}\sin(\omega t)\) 随时间同频交变，磁场幅值的空间分布仍保持 \(1/r\) 衰减律，只是整体随时间做工频振荡。

图~\ref{fig:ch02-single-phase-field} 把上述解析关系画成横截面场图。圆点和叉号分别表示电流沿电缆轴正向和反向：电流过零并换向后，环向磁场同步反向，而相同电流绝对值下的幅值等值线保持不变。下方双对数曲线进一步显示，观测半径增大 10 倍时场强降为原来的 \(1/10\)。因此，单相场的``方向''由右手定则确定，``强弱''则由观测距离决定，两者在几何上可以清楚分开。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.96\linewidth,height=0.72\textheight,keepaspectratio,alt={单相交流直导线在正负半周的环向磁场方向以及磁场幅值随径向距离按一阶反比衰减的独立仿真图}]{magnetics/single_phase_field_direction.pdf}
\caption{单相交流直导线磁场方向与径向衰减：正、负半周仅改变环向场方向，幅值空间分布均服从 \(1/r\) 规律；图中取单位峰值电流，仅用于定性说明}\label{fig:ch02-single-phase-field}
\end{figure}
\FloatBarrier
...
\label{eq:ch02-three-phase-current}
\end{equation}
...
为直观显示这种叠加，图~\ref{fig:ch02-three-phase-helical-field} 对无铠装、完全对称的三相螺旋芯线进行了独立离散仿真。仿真取芯线分布半径 \(a=0.06\text{ m}\)、绞距 \(p=1.0\text{ m}\)，沿 7 个绞距把导体中心线离散成有限电流元，再逐项计算毕奥--萨伐尔积分；图中只展示远离两端的中央 3 个绞距。固定时刻有 \(i_A=I_m\)、\(i_B=i_C=-0.5I_m\)，尽管三相电流代数和为零，但三个电流源不在同一空间位置，外部场并不会处处消失，而是形成比单芯场更弱且方向更复杂的漏磁场。该图用于解释场形与方向，不包含铠装、制造偏差、海床介质和背景噪声，因而不作为真实海缆绝对场强的预测。

\begin{figure}[H]
\centering
\includegraphics[width=0.96\linewidth,height=0.62\textheight,keepaspectratio,alt={平衡三相螺旋导体的几何、横截面漏磁方向以及沿绞距变化的三轴磁场分量和均方根模量}]{magnetics/three_phase_helical_field.pdf}
\caption{平衡三相螺旋电缆的定性磁场仿真：（a）三芯螺旋几何与观测线上的瞬时磁场方向；（b）尽管 \(i_A+i_B+i_C=0\)，空间分离的三芯在外部仍产生漏磁；（c）彩色瞬时分量随轴向旋转，而黑色三轴 RMS 合成强度保持近似稳定}\label{fig:ch02-three-phase-helical-field}
\end{figure}
...
\label{eq:ch02-shield-equivalence}
\end{equation}
...
由于三相芯线沿轴线以绞距 \(p\) 螺旋缠绕，空间磁场矢量的方向 \((\hat{B}_x, \hat{B}_y, \hat{B}_z)\) 沿轴向呈周期性旋转（理想模型中的空间周期为绞距 \(p\)）。图~\ref{fig:ch02-three-phase-helical-field}(c) 中三个瞬时分量反复换向，直观表明单一分量或瞬时矢量方向会随螺旋相位改变。若直接采用该方向进行横向偏航指引，控制系统便可能产生与芯线绞合周期对应的伪转向震荡，这正是本文不直接采用``瞬时矢量方向跟踪''的物理原因。

解决之道是转而求解三轴磁场工频幅值的均方根模量（RMS Total Magnitude）：

\begin{equation}
|\boldsymbol{B}_{f_0,\mathrm{rms}}(r)| = \sqrt{A_{x,f_0}^2 + A_{y,f_0}^2 + A_{z,f_0}^2}
\label{eq:ch02-rms-magnitude}
\end{equation}

\textbf{理想对称模型性质（模量旋转不变性）}：对于无限周期、三芯几何完全对称且三相电流严格平衡的模型，轴向平移一个螺旋相位等价于源几何和磁场矢量的共同旋转；旋转改变三轴分量，却不改变三轴 RMS 模量。因此在径向距离 \(r\) 相同时，\(|\boldsymbol{B}_{f_0,\mathrm{rms}}(r)|\) 对轴向螺旋相位保持不变。有限长度、芯线不对称和外部介质会破坏严格对称性，使实际包络保留小幅轴向纹波；图~\ref{fig:ch02-three-phase-helical-field}(c) 的有限长度离散模型在中央三绞距内给出约 \(1.55\%\) 的最大相对起伏。结合 2.3.2 节远场偶极子主导的近似，模量包络可进一步写成随径向距离单调衰减的二维曲线：

\begin{equation}
|\boldsymbol{B}_{f_0,\mathrm{rms}}(r)| \approx \frac{C_{dipole}}{r^2}, \qquad C_{dipole} = \frac{\mu_0}{2\pi}\,\eta\, m_{eff}
\label{eq:ch02-farfield-envelope}
\end{equation}

这一性质的工程意义在于：在理想对称和远场条件下，可以把复杂的三维螺旋矢量场约化为近似稳定的二维 \(1/r^2\) 径向包络，显著降低螺旋相位对定位量的影响。实验室双芯偶极子 + 坡莫合金屏蔽台架据此用于复现主要远场包络，而不是复刻真实三相铠装海缆的全部局部场细节。2.3.5 节的深度反演关系建立在这一远场、单峰且包络稳定的工作假设上。
...
\label{eq:ch02-fwhm-depth}
\end{equation}

图~\ref{fig:ch02-fwhm-depth-principle} 从两个互补角度显示这一步为什么成立。左图保持每条曲线峰值归一化，垂直距离 \(z\) 增大时横向响应按相同比例展宽，半峰交点始终位于 \(x=\pm z\)；右图固定 \(z\) 而改变尺度项 \(C=\mu_0m_{eq}/(2\pi)\)，峰值虽随电流、等效偶极矩或屏蔽衰减改变，两个半峰交点却保持不动。因此 FWHM 提取的是曲线的横向几何尺度，而不是绝对场强。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.96\linewidth,height=0.48\textheight,keepaspectratio,alt={不同垂直距离下偶极子模量包络的半高全宽变化，以及不同幅值尺度下半高全宽保持不变}]{magnetics/fwhm_depth_principle.pdf}
\caption{偶极子模量包络的 FWHM 深度几何：（a）垂直距离决定剖面宽度并满足 \(W_x=2z\)；（b）幅值尺度 \(C\) 仅改变峰高，不改变半峰交点}\label{fig:ch02-fwhm-depth-principle}
\end{figure}
\FloatBarrier
...
\label{eq:ch02-lever-arm}
\end{equation}

对埋深反演最关键的是垂直分量。取式\eqref{eq:ch02-lever-arm}第三分量做改正：

\begin{equation}
z_{CRP} = z_{raw} - [0,\ 0,\ 1]\cdot(\boldsymbol{R}_b^n \cdot \boldsymbol{l}_{mag}^b)
\label{eq:ch02-lever-arm-z}
\end{equation}
...
\label{eq:ch02-lever-arm-expand}
\end{equation}
...
\toprule\noalign{}
机动工况 & 校正前几何伪跳变（cm） & 校正后残差（cm） & 校正前模量伪波动（nT） & 校正后残差（nT） \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
机动工况 & 校正前几何伪跳变（cm） & 校正后残差（cm） & 校正前模量伪波动（nT） & 校正后残差（nT） \\
...
\bottomrule\noalign{}
\endlastfoot
纯俯仰 ±5° & 8.96 & \textless{} 0.32 & 320 & \textless{} 0.41 \\
纯横摇 ±5° & 1.31 & \textless{} 0.11 & 45 & \textless{} 0.12 \\
复合机动 & 9.85 & \textless{} 0.38 & 355 & \textless{} 0.48 \\
\end{longtable}

在复合机动工况下，几何伪跳变从 9.85 cm 压制到 0.38 cm（约为 0.2 m 埋深标准的 1.9\%），验证了杆臂改正与安装角校正模型的有效性。需要说明的是，上表数据来自实验室转台缩比标定，用于验证标定方法与预期精度；真实 AUV 在海试条件下的杆臂/安装角标定仍属实物部署阶段工作，故上表第~\ref{sec:ch02-2-4-2}节三类标定项的``当前状态''仍标注为``待实验''，与后续状态估计与实验章的口径保持一致。
...
在 UDP 二进制协议之下，还有承载它的物理总线与时钟基准。本系统在舱内部署一块微型 5 口千兆交换机 PCBA，把执行脑、决策脑、DVL、声呐、上位机组织为统一子网 \texttt{192.9.0.X}（执行脑 AMD \texttt{.50}、决策脑 Jetson \texttt{.100}、DVL PA600 \texttt{.60}、EdgeTech 2205 声呐 \texttt{.101}、GCS \texttt{.200}），交换机采用全双工非阻塞结构、总背板带宽 10 Gbps。这里的链路规格不等于应用层控制时延；真实 PC104 的周期调度与 host-relay 路径时序留待第 5 章的实物时序实验单独实测。舱壁穿舱采用 SubConn DBH13M 水密接口，其引脚与信号映射如下表；穿舱本体经 M16×1.5 不锈钢葛兰头 + 环氧树脂真空灌胶密封，耐受 \(\geq 1.0\text{ MPa}\)（等效 100 m 水深）且零渗漏。
...
Pin12/13 & \texttt{Trig1\ /\allowbreak{}\ Trig\ GND} & 1PPS/外触发预留 / 信号地 \\
\end{longtable}

Pin12/13 为 1PPS 或外部触发同步预留，可用于把 DVL/GPS 秒脉冲送入声呐 \texttt{Trig1} 与 Jetson GPIO；本阶段尚未完成该线路的接线、时间戳捕获与同步误差标定，因而不报告微秒级跨设备绝对对时精度。当前实物时序采用 PC104 \texttt{tickGet()} uptime 回传与 Jetson 侧首帧锚定，具体实现、时钟速率差和适用边界见第 3.2.1 节，其实物记录与验证结果见第 5 章。

\subsection{电气隔离与 ±15V 隔离电源}\label{sec:ch02-2-5-2}
...
TMR8637 由三只正交布置的 TMR8607 单轴敏感单元构成；规格书对单轴单元给出的典型灵敏度为 \(20.00\text{ mV}/\mu\text{T}\)，典型测量范围为 \(\pm200\ \mu\text{T}\)，在 \(\pm15\text{ V}\) 供电时最大可达 \(\pm500\ \mu\text{T}\)\cite{multidimension2024tmr8607}。这些单轴参数用于确定模拟前端增益和防饱和边界。随实物提供的编号 6\# 模组测试报告补充了模组层证据：X、Y、Z 三轴灵敏度分别为 20.02、19.98 和 19.96 mV/\(\mu\)T，在 \(\pm200\ \mu\text{T}\) 测试区间内传输曲线近似线性；三轴在 1 Hz 处的报告噪声均为 200 pT/\(\sqrt{\text{Hz}}\)。报告中的三轴性能参数汇总见表~\ref{tab:ch02-tmr8637-test-data}，电压-磁场传输特性和本底噪声谱分别见图~\ref{fig:ch02-tmr8637-axis-report}和图~\ref{fig:ch02-tmr8637-noise-psd}。
...
量化步长略高于 0.05 nT 并不构成窄带检测的硬下限。噪声抖动使相邻码之间的统计信息能够被多样本估计利用，而相干积分进一步只保留目标频率附近的分量。在 16 kHz 采样、OSR=8、1 s 非重叠 45 Hz 正交解调条件下，短接 ADC 子链的等效磁噪声为 \(0.0230\text{ nT}\) 矢量 RMS，I/Q 单分量 \(3\sigma\) 为 \(0.0514\text{ nT}\)。这组结果说明所选 ADC 子链已进入 0.05 nT 灵敏度量级，并给出了采样率与数字处理参数的实验依据；它描述的是规定带宽和判据下的最小可辨变化，其可溯源的绝对不确定度预算与实测 ENOB 表征属实验章内容。
...
\label{eq:ch02-butterworth-enbw}
\end{equation}
...
通信和电气两个底层问题解决之后，剩下的是顶层的``权限''问题：谁可以向 PC104 下发指令？在自主水下系统中，``完全自主''听起来很有吸引力，但工程上必须保留人类接管权限，这既是对未知环境的兜底，也是法规和验收条件的硬性要求。本系统的 PySide6 上位机因而不是单纯显示界面，而是操作员授权、安全接管和任务配置节点。与之相对，Foxglove 面向 ROS2 运行话题的高维状态观察、回放和验收，Jetson 执行感知、决策、控制与仲裁，PC104/VxWorks 承担二进制协议接收、执行器驱动和本地失联保护；多消费者实物联调时，fan-out 只负责独占 \texttt{21/udp}、复制上行并对下行实施零执行器门控，不参与任务决策。三种上位机工作模式定义如下：
...
\bottomrule\noalign{}
\endlastfoot
MANUAL & UDP \texttt{\$CKTH}，可经 fan-out & 由操作员直接设置工作指令与执行量，用于人工接管和现场调试 \\
AUTONOMY & Zenoh 语义命令 + Jetson bridge & 下发任务与自主请求，经守卫和仲裁后由 Brain 执行 \\
ESTOP & 零执行器 \texttt{\$CKTH}；UDP 下发并可镜像至 Zenoh & 立即切回遥控、取消任务并锁存，解除前保持零输出 \\
\end{longtable}

图~\ref{fig:ch02-operator-console} 给出最终上位机主界面。界面从上到下依次组织实时遥测、地图与控制面板、航点/任务/消息/诊断页签，以及始终可见的 ESTOP、MANUAL/AUTONOMY 和任务参数区；Zenoh、传感器置信度与电缆监控被收纳到诊断页，从而避免低优先级信息挤压底部安全操作。该图由 \texttt{Xvfb + xdotool + ImageMagick} 在 \(1366\times900\) 虚拟屏幕下生成，采用本机 dummy UDP 回环且关闭 Zenoh，故它证明的是界面可无头复现、中文渲染正确且该分辨率下无明显裁切，不证明图中状态栏文字对应真实 PC104 在线或整艇闭环。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\linewidth,height=0.56\textheight,keepaspectratio,alt={PySide6 AUV 上位机主界面，顶部为实时遥测，中部为地图和通信任务控制，下部为航点页签、急停、手动自主授权及任务参数}]{system/auv_console_operator_interface.png}
\caption{PySide6 操作员上位机的最终布局（Xvfb、dummy UDP 回环、Zenoh 关闭）：遥测、地图、任务控制和诊断分区围绕底部常驻的急停与模式授权组织}\label{fig:ch02-operator-console}
\end{figure}

ESTOP 的实现进一步体现了``一条安全意图、分层到达''：按钮点击后，上位机立即构造控制模式 \texttt{0x01}、工作指令 \texttt{0x02}、主/侧推与四舵全零的 72 B \texttt{\$CKTH}，并在锁存期间由 600 ms 发送定时器重复该帧。WiFi 通信管理器先把原始帧发往 UDP 目标；若 Zenoh side channel 已连接，再把同一帧镜像到 \texttt{rt/pc/cmd\_raw}，由 Jetson bridge 解码并触发 Remote/ESTOP 锁定。因此这两条传输路径形成了实现层冗余，但并非两套彼此独立生成的急停语义。第 5 章的实物记录已经证明 GUI ESTOP 点击可使真实 PC104 的 \texttt{work\_cmd} 变为 \texttt{0x02} 且执行量保持为零，并另行证明通用原始帧可以到达 Zenoh probe；GUI ESTOP 经生产 bridge/arbiter 消费的同次端到端语义闭环仍作为边界保留。

VxWorks 失联保护则是这套安全闭环的最后一层冗余，当 PC104 在 Jetson 控制模式下约 1 秒未收到有效指令时，会自发切回遥控模式、清零推进并置通信超时告警。这一机制不依赖上位机的主动指令，而是 AMD 端自己的``看门狗式''保护，覆盖了``上位机断电、Jetson 死机、网络全部中断''等极端情形。需要强调的是，零推力回退与主动上浮并不是同一类动作：前者对应控制权或命令链失效，后者对应深度越限、近底或 DVL 失锁等由 PC104 本地检测到的航行安全风险。把这四道防线（行为树 failsafe、安全仲裁器、上位机 ESTOP 分层下发、VxWorks 失联保护）合在一起看，它们构成一条``由内向外、由软到硬''的安全梯度；各层降低了单点故障直接穿透至执行器的风险，但其联合有效性仍须按各自证据层级而非抽象冗余数量判断。四类守卫的触发优先级及其到安全目标态的仲裁关系如图~\ref{fig:ch02-emergency-arbitration}所示；任务执行期间由操作员、仲裁器、决策脑和执行脑共同参与的完整生命周期如图~\ref{fig:ch02-mission-lifecycle}所示。
