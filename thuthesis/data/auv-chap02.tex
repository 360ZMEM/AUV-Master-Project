...
Pin12/13 为 1PPS 或外部触发同步预留，可用于把 DVL/GPS 秒脉冲送入声呐 \texttt{Trig1} 与 Jetson GPIO；本阶段尚未完成该线路的接线、时间戳捕获与同步误差标定，因而不报告微秒级跨设备绝对对时精度。当前实物时序采用 PC104 \texttt{tickGet()} uptime 回传与 Jetson 侧首帧锚定，具体实现、时钟速率差和适用边界见第 3.2.1 节及第~\ref{sec:ch05-5-3-1} 节。
