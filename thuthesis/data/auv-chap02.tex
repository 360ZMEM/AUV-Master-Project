...
上述几何关系的直观含义如图~\ref{fig:ch03-active-crossing} 所示。AUV 以横切角 \(\theta\) 反复穿越电缆，每次穿越在真值电缆上留下一个峰值观测点：峰值时刻定位横向中心线 \(e=0\)，半高全宽 \(W_x\) 反演垂直距离 \(z\)，一次横切即同时给出横向与垂直两类几何观测；多次穿越的峰值点再经加权拟合得到局部估计中心线。需要区分的是，这里描绘的是跟踪与重捕阶段的\textbf{局部主动探查}，其目的是在小范围内构造可观测性，而非搜索阶段用于大范围覆盖的之字形测线。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\linewidth,height=\textheight,keepaspectratio,alt={主动横切构造几何可观测性：上图为俯视几何，峰值观测点定位横向中心线，下图为单次横切的模量剖面，峰值定位横向中心、FWHM反演垂直距离}]{architecture/cable_active_crossing_observability.png}
\caption{主动横切构造几何可观测性：一次横切同时给出横向中心线（峰值时刻 \(e=0\)）与垂直距离（半高全宽 \(W_x=2z\)），对应跟踪/重捕阶段的局部主动探查}\label{fig:ch03-active-crossing}
\end{figure}
