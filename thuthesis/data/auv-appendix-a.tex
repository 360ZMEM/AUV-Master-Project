...
\begin{equation}
  ...
  \label{eq:app-control-rate}
\end{equation}
求解回退率定义为
\begin{equation}
  r_{\mathrm{fb}}=
  \frac{N_{\mathrm{fb}}}{N_{\mathrm{ctrl}}},
  \label{eq:app-fallback-rate}
\end{equation}
...
\begin{equation}
  ...
  \qquad
  h_{\min}=\min_k h_k .
  \label{eq:app-terrain-rmse}
\end{equation}
对海缆巡检，令 \(d_{\perp,k}\) 为相对电缆中心线的横向偏移，\(s_k\) 为沿先验路由的累计进度，\(\hat d_k\) 与 \(\sigma_{d,k}\) 分别为埋深估计及其标准差。上述变量共同构成巡检质量与验收判断的最小数据契约。

双脑通信的学术数据契约只要求下行数据包含任务模式、控制意图和执行参数，上行数据包含姿态、速度、深度、能源状态与故障状态。具体字段偏移和程序接口属于实现文档，不进入论文附录；实物证据分别报告上行到达间隔、应用路径 RTT、板内处理时间、受控丢包/延迟和序列号跳变统计，不把这些异质口径相加或改写为严格单向物理时延。
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.235\linewidth}>{\raggedright\arraybackslash}p{0.185\linewidth}>{\raggedright\arraybackslash}p{0.285\linewidth}>{\raggedright\arraybackslash}p{0.235\linewidth}@{}}
...
UA-MPC 可解性归因 & 可解性专项三变体 27 次；A1 默认层 9 次 & S 型控制代价降权为可解性主贡献 & 失败耗时尚未覆盖全部回退类型 \\
...
PySide6 上位机分阶段联调 & GUI-only 截图、2 次实物按钮动作、Zenoh 16 帧调试摘要 & 无头界面可复现；按钮字段到达真实 PC104；原始帧可镜像到 Zenoh & 非执行器动作或 GUI--ROS2--PC104 同次闭环；部分阶段原始日志未归档 \\
PC104 零执行器故障矩阵 & 5 次自然断链、3 次强制超时及丢包/延迟/Bit5 各一阶段 & 真实板卡的 watchdog、Remote 降级、故障位回传与恢复成立 & 板端基线；非执行机构响应，同步跨层结论由下项独立运行提供 \\
PC104--ROS2--BT 同步安全链 & 修正后短时 run 18/18；独立 1800 s 稳态 & Bit5/13/14 可驱动仲裁与任务降级；稳态 ACTIVE 比率 1.0、目标故障状态为 0 & 两个独立 run；零执行器、DVL/电压隔离；非 Jetson thermal soak 或水域闭环 \\
生产仲裁器与行为树回放 & 仲裁器 10 ms 步长；BT 9 场景 \(\times\) 3 种子 & MPC/PC 超时零输出与任务级降级可重复 & 固定规则基线，不作为同步实机时延 \\
...
专用仓库算法级电缆边界 & 多场景确定性单次扫描（\(n=1\)） & 在线先验修正与自适应之字形为关闭即失败机制；投影连续与物理配准解耦；调优后之字形埋深进入 0.15 m 参考线 & 纯仿真单次证据，不等同主仓端到端、多种子或实物；30 m 为扫描下限非失效阈值（详见第~\ref{sec:app-a-9}节） \\
\end{longtable}
\endgroup
...
\bottomrule\noalign{}
\endlastfoot
基线定位（多种子） & \(n=3\) & 基线定位指标均值±标准差可用 & 仅限 30 s 片段 \\
...
UA-MPC 机制消融与迭代上限对照 & 可解性/最大迭代/高机动多档对照 & sigmoid 控制代价降权是可解性与横向精度主贡献 & 单纯提高迭代上限会阻塞控制周期 \\
分源 NIS 审计 & 8 场景×3 种子，35243 次量测更新 & 深度与 DVL 一致性偏差方向相反（见第~\ref{sec:ch05-5-5-5}节） & 历史混合阈值只作操作性触发 \\
ES-EKF 五分钟时序敏感性 & 3 场景×5 种子×2 相位，30 次运行 & 成对噪声下隔离 0/100 ms DVL 相位效应（见第~\ref{sec:ch05-5-5-5}节） & 离线动力学；无持续绝对水平位置观测 \\
实测背景噪声回放 & 同协方差高斯/实测回放×5 种子 & 均值接近，实测时序显著增大跨种子离散（见第~\ref{sec:ch05-5-5-5}节） & 3.92 s 记录循环，不支持长时平稳性 \\
滤波策略与时间戳基线 & 3 场景×6 策略×5 种子，90 次运行 & Huber 更新显著抑制脉冲异常；分源 R 避免跨源混合（见第~\ref{sec:ch05-5-5-5}节） & 离线动力学；时间戳膨胀不等同延迟状态更新 \\
基线定位/控制/决策 & \(n=1\) & 工具链与单次基准可用 & 不能替代多种子 \\
地形跟随 PID/MPC & \(n=1\)/组 & PID 地形跟随是当前近底主结果 & 需重复统计 \\
PID 地形跟随三档（多种子复验） & \(n=3\)/档 & 三档均三次全部成功、零安全违规、离底 RMSE 0.59--0.71 m（见第~\ref{sec:ch05-5-5-3}节） & 部分档合并了重跑种子 \\
深度 MPC 调参 & 多轮调参 & 深度 MPC 不作为主线 & 只能写回退理由 \\
MPC 极端平面路径 & \(n=1\)+\(n=5\) 种子/场景 & 公平口径下 MPC 在多数极端路径优于或持平基线，受控扰动五种子统计复核（见第~\ref{sec:ch05-5-5-4}节） & 仅直角路径 LOS 更优（诚实边界） \\
TMR8637 随附测试报告 & 1 份报告/编号 6\# 模组三轴 & 1 Hz 为 200 pT/\(\sqrt{\text{Hz}}\)，50 Hz 约 20--30 pT/\(\sqrt{\text{Hz}}\)；模组本体具备窄带噪声裕度（见第~\ref{sec:ch05-5-3-2}节） & 供应方随附单件报告，非本文独立复测，非完整采集链 \\
SK2301 短接 ENOB 与 45 Hz 锁相 & 4 档采样率×10 s，最高 160000 样本 & ENOB 16.877--18.342 bit；45 Hz 矢量 RMS 0.0230 nT（见第~\ref{sec:ch05-5-3-2}节） & ADC 子链灵敏度，不等同整机绝对精度 \\
ADC--TMR 全链路宿舍背景回放 & 2 段默认背景记录，共 13.104 s；17 段 raw data 全量审计 & 45 Hz 向量锁相中位 2.584 nT、50 Hz 工频中位 169.284 nT；45 Hz Biot--Savart 注入回放幅值 RMSE 0.792/0.271 nT（见第~\ref{sec:ch05-5-3-2}节） & 非屏蔽宿舍环境，不能替代实验室本底计量；用于半实物噪声回放与压力测试 \\
ADC--TMR 实测背景 ROS2 闭环回放 & 清洁/同协方差高斯/实测回放三臂各 20 s & 噪声只进入磁观测量测副本；实测回放横偏 p95 为 7.767 m、平均置信度 0.8477、控制变化率 RMS 0.06346，未改变同配置闭环主导状态（见第~\ref{sec:ch05-5-3-2}节） & Direction A 解耦轻量闭环门禁，非完整 AUV 水下实测；由于大横偏和埋深方差缺失，不作为行业验收通过率实验 \\
45 Hz 电缆--TMR--ArUco 联合反演 & 106821 ADC 样本，777 对齐点 & 复 I/Q 自由/固定电流拟合 \(R^2=0.8792/0.8711\)（见第~\ref{sec:ch05-5-4}节） & 直导线与公共复尺度模型 \\
磁传感器杆臂/安装角标定（仿真） & \(n=1\) 全流程 & 杆臂改正链路与文件契约成立，平移降 96.27\%、旋转降 95.31\%、通过（见第~\ref{sec:ch05-5-3-3}节） & 仿真标定框架、非真机转台、未估九参数 \\
代理电缆六场景正式矩阵 & 6 场景×2 模式×3 种子 & 36/36 完成、回退为 0；保守 UA 将全局控制变化率 RMS 由 0.247 降至 0.085（见第~\ref{sec:ch05-5-7-7}节） & 横向 RMSE 未系统改善；代理配置不代表原生声磁闭环 \\
CBF 安全过滤低/中/高三种子矩阵 & 低/中/高 \(\times\) 3 种子 \(\times\) PID/MPC，各约 60 s & 18/18 运行有效；最小净空 2.70--2.90 m，1.5 m 低净空违规率和穿底率均为 0；MPC 回退为 0，求解耗时 p95 最大 23.84 ms（见第~\ref{sec:ch05-5-7-7}节） & 手动设定点加运动学代理口径；不代表 PVS 原生执行链或所有极端场景 \\
原生电缆因子生成器 & 16 组合×2 种子，32 次契约验证 & 几何、地形、实测噪声与横流可独立开关，真值与量测隔离 & 生成器验证，不替代控制闭环因子交互实验 \\
原生因子闭环全矩阵 & \(2^4\) 组合×3 种子×2 模式，96 次 20 s & 96/96 有效，回退与安全违规均为 0；最坏横向 p95 为 1.716 m，最小净空 2.536 m；求解耗时 p95 最大 24.02 ms（见第~\ref{sec:ch05-5-7-7}节） & 离线仿真代理口径；非 PVS 原生执行链或真机物理时延 \\
原生因子闭环 60 s 长时复核 & \(2^4\) 组合×3 种子×2 模式，96 次 60 s & 96/96 有效，回退与安全违规均为 0；最坏横向 p95 为 2.176 m，最小净空 2.384 m；求解耗时 p95 最大 14.93 ms（见第~\ref{sec:ch05-5-7-7}节） & 未触发路径终点；仍非 PVS 原生执行链/真机 \\
原生因子闭环地形加横流 120 s 终端边界测试 & 4 组合×3 种子×2 模式，24 次 120 s & 24/24 有效，安全违规为 0；全窗口最坏横向 p95 为 6.839 m；终端审计显示活动巡缆段 p95 最大 2.152 m、终端尾段 p95 最大 7.703 m；UA 回退最大 0.113（见第~\ref{sec:ch05-5-7-7}节） & 终端任务边界；不能写成活动段跟踪崩溃或长航时定点保持 \\
原生因子闭环 120 s 任务层终端退出验证 & 同上，到达路径末端即退出 & 24/24 有效且均到达路径末端；退出后最坏横向 p95 为 2.152 m，UA 回退率最大 0.018；solver p95/max 最大 22.99/53.27 ms（见第~\ref{sec:ch05-5-7-7}节） & 消除终端尾段指标污染；不代表定点保持或 PVS 原生执行链 \\
PVS 原生执行链冒烟 & 约 50 s 定深定向仿真 & 审计通过；PVS 原生定深定向自驾配置，AUTO 日志 149 次，运动学日志为 0；设定点/控制/状态帧分别 245/720/998，solver p95/max 为 19.32/20.40 ms（见第~\ref{sec:ch05-5-7-7}节） & 执行链证据；非原生因子闭环性能矩阵、真机或海试 \\
PySide6 GUI 无头复现与实物按钮字段 & 最终 \(1366\times900\) dummy 截图；真实 ``主推上电''/ESTOP 各 1 次；Zenoh Stage 4 摘要 16 帧 & 布局可复现；PC104 Telnet 的 \texttt{work\_cmd} 依次为 \texttt{0x11}、\texttt{0x02}，且电机为零；原始 \texttt{\$CKTH} 可到 Zenoh probe（见第~\ref{sec:ch05-5-3-1}节） & dummy 截图非在线证据；Stage 2--4 原始日志不完整；Zenoh 未由生产 bridge/arbiter 做 ESTOP 同次语义验收 \\
PC104 UDP 时序与固件回显 & 300 s 主机中继安全零推力实机记录 & 下行 3000 帧、解析 4505 帧 \texttt{\$AUV} 上行，解析错误与前向序号缺口均为 0；1001 个可配对固件接收事件的 first-echo RTT p50 为 264.678 ms、p95 为 313.965 ms、p99 为 315.607 ms，receive-to-pack p95 为 16 ms（见第~\ref{sec:ch05-5-3-1}节） & host-relay 应用路径 RTT，包含容器与固件周期调度；非单向物理时延或执行器响应 \\
PC104 telnetd 零执行器故障注入 & 79.672 s，1197 帧上行；5 次自然断链、3 次强制超时 & 自然/强制超时检测 5/5、3/3；自然 Bit14 触发 p50/p95 为 1044.111/1056.357 ms；Remote 降级、恢复、Bit5 回传与清除成立（见第~\ref{sec:ch05-5-3-1}节） & DVL 锁定和 Bit5 为临时运行时注入；全程零命令；非真实 DVL/MCU 故障或执行机构动态响应 \\
PC104 应用路径受控扰动 & 固定种子 100 帧丢包阶段及 10 s 延迟阶段 & 精确丢弃 30/100 帧；200 ms 队列延迟 p50/p95 为 200.979/203.996 ms；均未误触发 Bit14（见第~\ref{sec:ch05-5-3-1}节） & 注入点位于容器发送器至 host relay 之前，不是 Ethernet PHY 丢包或单向传播时延 \\
生产仲裁器超时回放 & 10 ms 步长、2.2 s 确定性时间轴 & MPC 在 510 ms 零输出回退，PC 在 1010/1510 ms 进入 WEAK/LOST 并切 Remote 零输出；本地降级不再误续 PC 心跳（见第~\ref{sec:ch05-5-3-1}节） & 直接执行生产核心但不含 ROS executor、网络或硬件调度 \\
行为树分层故障回放 & 9 场景 \(\times\) 3 种子，27 次 & 27/27 有效，漏报和假阳性为 0；覆盖授权丢失、感知降级、求解超时代理和紧急中止角色（见第~\ref{sec:ch05-5-3-1}节） & 固定规则与确定性基线；真实同步时延由下项给出 \\
PC104--ROS2--BT 同步故障链 & 修正后短时运行 18/18 项通过 & Bit13 到仲裁/BT 为 65.097/349.100 ms；Bit5 到 DENIED/IDLE 为 102.532/188.463 ms；真实 watchdog Bit14 到 DENIED 为 1071.706 ms；清除后须显式重授权（见第~\ref{sec:ch05-5-3-1}节） & 主机单调时钟触发时间；电压 guard 与 DVL 为安全隔离；不支持单向物理时延、真实电源/DVL 或执行器动态结论 \\
PC104--ROS2 零执行器稳态 & 独立 1800 s 稳态区间、30 个分钟级样本 & 124841 个 ArbiterStatus 样本 ACTIVE 比率为 1.0，目标故障状态、非零板端命令快照和非零推进反馈均为 0（见第~\ref{sec:ch05-5-3-1}节） & 原包后置旧 Bit14 序列有 1 项失败；稳态与修正后 18/18 故障链为两个 run；旧 RSS 是启动根进程口径，非 Jetson thermal soak \\
行为树 vs 状态机 & 1000 次延迟、100 条噪声序列、500 次组合故障 & 等价规则下响应和生存率相同；迟滞去抖降低颤振 & 结论限于决策核心仿真 \\
海缆行业标准数字孪生验收 & 3 次全新运行 & 全链路闭环、三次全部就绪且通过（见第~\ref{sec:ch05-5-5-10}节） & 数字孪生确定性先验，非真实检测噪声 \\
解耦轻量闭环 & 单次冒烟验证（\(n=1\)） & 满足磁观测前提时在线先验修正被真实观测接受，算法实机部署接口成立（见第~\ref{sec:ch05-5-5-11}节） & 轻量运动学闭环，无地磁背景/检测噪声/硬件时延/六自由度水动力 \\
六自由度闭环偏差先验恢复 & 中/重档各 3 次全新运行 & 在线修正被真实接受、六自由度闭环恢复得到复现，中/重档各三次全部就绪通过（见第~\ref{sec:ch05-5-5-11}节） & 数字孪生确定性先验、静态位姿扭曲、缆在车下满足前提、窗口内判定、每档三种子 \\
\end{longtable}
...
\begin{enumerate}
  ...
  \item \textbf{实物验收：}TMR8637 已有单件模组三轴随附测试报告，PC104 正常通信、Bit5/Bit13/Bit14 到 ROS2 仲裁与行为树的同步零执行器安全链及当前拓扑 30 min 稳态已经完成；完整声磁噪声、硬件标定、Jetson thermal soak、执行机构响应和水域试验仍未完成。
\end{enumerate}
...
第~\ref{sec:ch05-5-5-11}节 引用的算法级结论均来自专用磁探测仓库的确定性离线扫描，其证据契约在此补足，以便正文数字可逐项回溯。场景族的关键参数包括：巡检车道长度与 \(50/70\,\mathrm{m}\) 车道间距、U 形掉头半径、以及先验畸变的轻/中/重三档（对应式\eqref{eq:app-prior-distortion}中平移 \(\boldsymbol t\) 与旋转 \(\delta\psi\) 的取值梯度）。判据方面，单次运行的通过与否由该仓库的 DL/T 1278 风格复合门限给出，其中跨车道跳变计数定义为沿先验路由的单步进度增量超过跨车道判定阈值的次数，用于捕获错位先验在 U 弯处诱发的跨车道投影跳变。部署接口的两组量语义不同且不可混用：地图系投影连续量描述投影落在先验哪一段，即使任务已失败其单步跳变仍可保持在约 \(0.2\,\mathrm{m}\)；先验配准量描述把错位先验拉向真实电缆的物理平移/旋转配准量，是任务能否恢复的真正状态。故``投影连续''``物理配准''``任务完成''是三个不同概念，不能相互替代。曲率一项，\(30\,\mathrm{m}\) 是扫描环境的硬下限而非已击穿的失效阈值：在 \(30\text{--}120\,\mathrm{m}\) 范围内未观测到弯道触发的失效，瓶颈接近电缆几何物理下限。真值字段（如真实电缆最近点坐标）仅进入离线评价，不进入在线感知管线。上述扫描随机种子未透传、当前为 \(n=1\)，因此只能作为同源算法的鲁棒性边界，不能改写为主仓端到端、多种子或实物结论。

该算法级证据的最小文件集、结果数据表、重绘脚本与迁移到本文的方法图/因果图，均随成果清单归档并带校验值，可据以逐项回溯正文引用的算法级数字，其证据等级为专用仓库、纯仿真、单次复现。
...
本表集中列出各问题域的边界归因、已完成处置、可写结论和剩余验证。正文结果小节只保留其结论所必需的限定；剩余验证的分层推进见第~\ref{sec:ch06-6-3}节。

\begingroup
\small
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.08}
\begin{longtable}[]{@{}>{\raggedright\arraybackslash}p{0.16\linewidth}>{\raggedright\arraybackslash}p{0.28\linewidth}>{\raggedright\arraybackslash}p{0.31\linewidth}>{\raggedright\arraybackslash}p{0.20\linewidth}@{}}
...
ES-EKF 统计一致性 & 历史实现把一维深度 NIS 与三维 DVL NIS 混入同一窗口；时间差膨胀 \(R\) 不能消除相位与积分漂移；PC104 与 Jetson 长期跨时钟域映射尚未估计斜率 & 固定/全局/分源/Huber 对照与分源一致性 A/B 已完成；PC104 打包/DVL 解析 uptime 已接入 ROS Header 和 DVL 延迟加权，300 s 时钟拟合给出约 \(+962\) ppm 相对速率差，但本轮有效 DVL 时间样本为 0 & 激活真实 DVL 后复验时间戳收益；增加在线时钟斜率估计、延迟状态更新与声磁绝对水平观测闭环 \\
...
不确定性感知控制证据 & 旧代理闭环高回退来自硬俯仰饱和与低频快照口径；保守分源 UA 主要改变控制风格，未系统改善横向 RMSE；综合极端因 \(p_{\mathrm{track}}\) 未越过进入门而不进入跟踪态，原 36 次未落盘速度/授权遥测 & 有界可导饱和后 36 快照全收敛、18 组关键闭环零回退；主结论收束为控制平滑与实时可解性（控制变化率 RMS 0.247→0.085），RMSE 边界由表~\ref{tab:ch05-r13-v2-main-results} 给出；补 12 次遥测证明速度缩放，但不回写正式矩阵结论 & 重构分源置信接口并重跑主矩阵；提升综合极端观测质量；正式矩阵内落盘速度门控与授权遥测，并在真机口径下复验精度效应 \\
CBF 与原生因子闭环 & CBF 矩阵仍是手动设定点加运动学代理；原生因子闭环全矩阵为离线代理，PVS 原生冒烟仅覆盖定深定向；120 s 固定时长会把参考路径完成后的尾段污染进巡缆指标；两者 p95 上界约 24 ms，不等于 10 ms 硬实时 & CBF 18/18 有效，最小净空 2.70--2.90 m，违规和穿底为 0；原生因子闭环 96/96 有效、回退和安全违规为 0，活动段 p95 最大 2.152 m，任务层到达即退出后 24/24 退出；相对 20 Hz/10 Hz/0.3 s 三级周期预算未越界 & PVS 原生全因子安全/性能矩阵、速度/横流/初始偏差边界搜索、终端保持/横流悬停控制、长航时稳定闭环与真机近底复验 \\
实物通信与端到端时延 & 300 s 固件回显已给出 host-relay 应用路径 RTT 和 PC104 内部接收至打包时间，但两端无共享时钟，不能把 RTT 拆成上下行单向物理时延；固件网络接收任务当前为 300 ms 周期；GUI Stage 2--4 部分原始日志未归档，Zenoh 只到 probe & 正常链路记录 3000 帧下行与 4505 帧上行，first-echo RTT p95 为 313.965 ms、receive-to-pack p95 为 16 ms；PC104 故障字--ROS2 仲裁--BT 同步状态链 18/18 通过，独立 1800 s 稳态 ACTIVE 比率为 1.0；全部为零执行器 & 接通 GUI ESTOP 到生产 bridge/arbiter；激活真实电压、DVL 与 ADC--EKF--控制全执行量链；评估 3.33 Hz 接收节拍对 0xEF 透传控制的影响；Jetson thermal soak、执行器响应与水域试验；如需单向时延须增加共享时钟或双向时间同步 \\
\end{longtable}
\endgroup

表中各结论均受其对应证据等级约束：仿真、数字孪生、数据回放、零执行器实物链路和执行器/水域试验不可互相替代。未完成项包括统计标定、真实传感与执行量链、Jetson 热稳定、系统级计量和水域试验。
