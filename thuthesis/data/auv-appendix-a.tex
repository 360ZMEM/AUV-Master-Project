...
PySide6 GUI 无头复现与实物按钮字段 & 最终 \(1366\times900\) dummy 截图；真实 ``主推上电''/ESTOP 各 1 次；Zenoh Stage 4 摘要 16 帧 & 布局可复现；PC104 Telnet 的 \texttt{work\_cmd} 依次为 \texttt{0x11}、\texttt{0x02}，且电机为零；原始 \texttt{\$CKTH} 可到 Zenoh probe（见 \S\ref{sec:ch05-5-3-1}） & dummy 截图非在线证据；Stage 2--4 原始日志不完整；Zenoh 未由生产 bridge/arbiter 做 ESTOP 同次语义验收 \\
PC104 UDP 时序与固件回显 & 300 s 主机中继安全零推力实机记录 & 下行 3000 帧、解析 4505 帧 \texttt{\$AUV} 上行，解析错误与前向序号缺口均为 0；1001 个可配对固件接收事件的 first-echo RTT p50 为 264.678 ms、p95 为 313.965 ms、p99 为 315.607 ms，receive-to-pack p95 为 16 ms（见 \S\ref{sec:ch05-5-3-1}） & host-relay 应用路径 RTT，包含容器与固件周期调度；非单向物理时延或执行器响应 \\
PC104 telnetd 零执行器故障注入 & 79.672 s，1197 帧上行；5 次自然断链、3 次强制超时 & 自然/强制超时检测 5/5、3/3；自然 Bit14 触发 p50/p95 为 1044.111/1056.357 ms；Remote 降级、恢复、Bit5 回传与清除成立（见 \S\ref{sec:ch05-5-3-1}） & DVL 锁定和 Bit5 为临时运行时注入；全程零命令；非真实 DVL/MCU 故障或执行机构动态响应 \\
PC104 应用路径受控扰动 & 固定种子 100 帧丢包阶段及 10 s 延迟阶段 & 精确丢弃 30/100 帧；200 ms 队列延迟 p50/p95 为 200.979/203.996 ms；均未误触发 Bit14（见 \S\ref{sec:ch05-5-3-1}） & 注入点位于容器发送器至 host relay 之前，不是 Ethernet PHY 丢包或单向传播时延 \\
生产仲裁器超时回放 & 10 ms 步长、2.2 s 确定性时间轴 & MPC 在 510 ms 零输出回退，PC 在 1010/1510 ms 进入 WEAK/LOST 并切 Remote 零输出；本地降级不再误续 PC 心跳（见 \S\ref{sec:ch05-5-3-1}） & 直接执行生产核心但不含 ROS executor、网络或硬件调度 \\
行为树分层故障回放 & 9 场景 \(\times\) 3 种子，27 次 & 27/27 有效，漏报和假阳性为 0；覆盖授权丢失、感知降级、求解超时代理和紧急中止角色（见 \S\ref{sec:ch05-5-3-1}） & 固定规则与确定性基线；真实同步时延由下项给出 \\
PC104--ROS2--BT 同步故障链 & 修正后短时运行 18/18 项通过 & Bit13 到仲裁/BT 为 65.097/349.100 ms；Bit5 到 DENIED/IDLE 为 102.532/188.463 ms；真实 watchdog Bit14 到 DENIED 为 1071.706 ms；清除后须显式重授权（见 \S\ref{sec:ch05-5-3-1}） & 主机单调时钟触发时间；电压 guard 与 DVL 为安全隔离；不支持单向物理时延、真实电源/DVL 或执行器动态结论 \\
PC104--ROS2 零执行器稳态 & 独立 1800 s 稳态区间、30 个分钟级样本 & 124841 个 ArbiterStatus 样本 ACTIVE 比率为 1.0，目标故障状态、非零板端命令快照和非零推进反馈均为 0（见 \S\ref{sec:ch05-5-3-1}） & 原包后置旧 Bit14 序列有 1 项失败；稳态与修正后 18/18 故障链为两个 run；旧 RSS 是启动根进程口径，非 Jetson thermal soak \\
