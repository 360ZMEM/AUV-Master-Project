...
本节把全章实验的负结果、口径限制和不可外推项集中收束为问题域，而不是逐条复述前文结果。表~\ref{tab:ch05-negative-results}保留各类边界的原因、处置和剩余验证指向；正文各结果小节只保留正向主结果与最小必要限定，相同局限统一指向本表。需要说明的是，本表``剩余验证''列并非未知缺口清单，而是已按可独立交付的实物证据链拆解的推进路径——每项均可在当前仿真结论之上增量验证，其向实物层推进的路线见第~\ref{sec:ch06-6-3}节，不构成对已有结论的否定。
...
这些结果共同表明，当前方法的主要算法链与真实 PC104 零执行器同步安全链已经闭合，剩余工作集中在统计标定、分源置信映射、原生耦合闭环、真实传感与执行量链、Jetson 热稳定和系统级计量验证。它们限定结论的适用范围，但不改变本文关于低成本窄带采集、声磁协同估计、不确定性感知控制和闭环恢复机制的核心论证。
